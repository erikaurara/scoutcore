...
export default function App() {
  ...
  useEffect(() => {
    ...
    const sharedProfileId = queryParams.get('profile');
    if (sharedProfileId && /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(sharedProfileId)) {
      setProfileLaunch({ profileId: sharedProfileId, view: 'discover', key: 1 });
  ...
  if (!accountSetupChecked) return <div className="flex min-h-screen items-center justify-center bg-[#07101f] text-[#dae2fd]"><div className="text-center"><img src="/ixmetrics-app-icon-192.png" alt="IXMetrics" className="mx-auto h-16 w-16 rounded-2xl" /><div className="mt-3 text-xs font-bold uppercase tracking-[.2em] text-[#00f0ff]">IXMetrics</div><div className="mt-1 text-[9px] font-semibold uppercase tracking-[.24em] text-[#8495aa]">AI Game Intelligence</div></div></div>;
