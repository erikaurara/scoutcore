...
export async function fetchTeamSeasonStats(teamId: number) { const season = new Date().getFullYear(); const data = await json(`${MLB_API}/teams/${teamId}/stats?stats=season&group=hitting,pitching&season=${season}`); const groups = data.stats ?? []; return { hitting: groups.find((entry: any) => entry.group?.displayName === 'hitting')?.splits?.[0]?.stat ?? {}, pitching: groups.find((entry: any) => entry.group?.displayName === 'pitching')?.splits?.[0]?.stat ?? {} }; }
