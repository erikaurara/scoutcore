...
function mapScheduleGame(game: any): MlbScheduleGame {
  ...
  return { gamePk: game.gamePk, gameDate: game.gameDate, status: game.status?.abstractGameState ?? 'Unknown', detailedState: game.status?.detailedState ?? 'Unknown', currentInning: game.linescore?.currentInning, currentInningOrdinal: game.linescore?.currentInningOrdinal, inningState: game.linescore?.inningState, awayTeam: { id: awayTeam.id, name: awayTeam.name ?? 'Away Team', abbreviation: awayTeam.abbreviation }, homeTeam: { id: homeTeam.id, name: homeTeam.name ?? 'Home Team', abbreviation: homeTeam.abbreviation }, awayScore: away.score, homeScore: home.score, awayProbablePitcher: away.probablePitcher?.id ? { id: away.probablePitcher.id, name: away.probablePitcher.fullName ?? 'TBD' } : undefined, homeProbablePitcher: home.probablePitcher?.id ? { id: home.probablePitcher.id, name: home.probablePitcher.fullName ?? 'TBD' } : undefined };
}
