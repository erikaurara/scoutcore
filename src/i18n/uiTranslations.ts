...
import { applyBrandCopy } from './brandCopy';
...
// IXMetrics. It lets legacy pages participate in the same language selector without
...
const TRANSLATION_ROWS: TranslationRow[] = [
  ...
  ['AI Game Intelligence', 'AI ゲームインテリジェンス', 'Inteligencia de juego con IA', 'AI 게임 인텔리전스', 'AI 比賽情報', 'Inteligência de jogo com IA', 'KI-Spielintelligenz'],
  ['Daily Intelligence', 'デイリーインテリジェンス', 'Inteligencia diaria', '일일 인텔리전스', '每日情報', 'Inteligência diária', 'Tägliche Analyse'],
  ['DAILY IXMETRICS INTELLIGENCE', 'IXMETRICS デイリーインテリジェンス', 'INTELIGENCIA DIARIA DE IXMETRICS', '일일 IXMETRICS 인텔리전스', 'IXMETRICS 每日情報', 'INTELIGÊNCIA DIÁRIA IXMETRICS', 'TÄGLICHE IXMETRICS-ANALYSE'],
  ...
  ['What is Daily Intelligence?', 'デイリーインテリジェンスとは？', '¿Qué es la Inteligencia diaria?', '일일 인텔리전스란?', '什麼是每日情報？', 'O que é a Inteligência diária?', 'Was ist die tägliche Analyse?'],
  ['This is IXMetrics’ signal board — not another schedule. It highlights verified matchup edges, recent hitter form, pitcher trends and bullpen watch items that may deserve a deeper look.', 'これは単なる日程表ではなく、IXMetrics のシグナルボードです。検証済みの対戦優位性、打者の直近調子、投手傾向、ブルペンの注目点を表示します。', 'Este es el tablero de señales de IXMetrics, no otro calendario. Destaca ventajas verificadas, forma reciente de bateadores, tendencias de lanzadores y alertas del bullpen.', '이 화면은 단순 일정이 아닌 IXMetrics 신호 보드입니다. 검증된 매치업 우위, 타자 최근 흐름, 투수 추세와 불펜 주의 항목을 보여줍니다.', '這是 IXMetrics 的訊號看板，不是另一個賽程表。它顯示已驗證的對戰優勢、打者近況、投手趨勢與牛棚關注項目。', 'Este é o painel de sinais do IXMetrics, não apenas outra agenda. Ele destaca vantagens verificadas, fase recente dos rebatedores, tendências de arremessadores e alertas do bullpen.', 'Dies ist das IXMetrics-Signalboard, kein weiterer Spielplan. Es zeigt verifizierte Matchup-Vorteile, aktuelle Batter-Form, Pitcher-Trends und Bullpen-Hinweise.'],
  ['Live MLB games · tap a game to open IXMetrics Gameday', 'MLBライブ試合 · タップしてIXMetrics Gamedayを開く', 'Partidos MLB en vivo · toca un partido para abrir el día de juego de IXMetrics', 'MLB 실시간 경기 · 탭하여 IXMetrics 경기 화면 열기', 'MLB 即時賽事 · 點選開啟 IXMetrics 比賽日', 'Jogos MLB ao vivo · toque para abrir o dia de jogo do IXMetrics', 'Laufende MLB-Spiele · antippen, um den IXMetrics-Spieltag zu öffnen'],
...
const translateDynamic = (source: string, locale: ScoutLocale): string | null => {
  ...
  const connectedGames = source.match(/^IXMetrics is connected directly to MLB data\.\s*(\d+) games are scheduled today(?:,\s*with\s*(\d+)\s*live)?\.?$/i);
  if (connectedGames) {
    const count = connectedGames[1];
    const live = connectedGames[2];
    return localized({
      ja: live ? `IXMetrics はMLBデータに直接接続されています。本日は${count}試合が予定され、現在${live}試合がライブ中です。` : `IXMetrics はMLBデータに直接接続されています。本日は${count}試合が予定されています。`,
      es: live ? `IXMetrics está conectado directamente a los datos de MLB. Hoy hay ${count} partidos programados, con ${live} en vivo.` : `IXMetrics está conectado directamente a los datos de MLB. Hoy hay ${count} partidos programados.`,
      ko: live ? `IXMetrics는 MLB 데이터에 직접 연결되어 있습니다. 오늘 ${count}경기가 예정되어 있으며 ${live}경기가 진행 중입니다.` : `IXMetrics는 MLB 데이터에 직접 연결되어 있습니다. 오늘 ${count}경기가 예정되어 있습니다.`,
      'zh-TW': live ? `IXMetrics 直接連接 MLB 資料。今天有 ${count} 場比賽，其中 ${live} 場正在進行。` : `IXMetrics 直接連接 MLB 資料。今天有 ${count} 場比賽。`,
      'pt-BR': live ? `O IXMetrics está conectado diretamente aos dados da MLB. Há ${count} jogos programados hoje, com ${live} ao vivo.` : `O IXMetrics está conectado diretamente aos dados da MLB. Há ${count} jogos programados hoje.`,
      de: live ? `IXMetrics ist direkt mit MLB-Daten verbunden. Heute sind ${count} Spiele angesetzt, davon ${live} live.` : `IXMetrics ist direkt mit MLB-Daten verbunden. Heute sind ${count} Spiele angesetzt.`,
    });
  }
  ...
  const watchGames = source.match(/^IXMetrics is watching (\d+) MLB games today$/i);
  if (watchGames) {
    const count = watchGames[1];
    return ({ ja: `IXMetrics は今日のMLB ${count}試合をチェック中`, es: `IXMetrics sigue ${count} partidos de MLB hoy`, ko: `IXMetrics가 오늘 MLB ${count}경기를 확인 중입니다`, 'zh-TW': `IXMetrics 正在關注今天的 ${count} 場 MLB 賽事`, 'pt-BR': `O IXMetrics acompanha ${count} jogos da MLB hoje`, de: `IXMetrics beobachtet heute ${count} MLB-Spiele` } as const)[locale as Exclude<ScoutLocale, 'en'>];
  }
  ...
  const signals = source.match(/^IXMetrics found (\d+) signals worth watching today$/i);
  if (signals) return localized({
    ja: `IXMetrics が今日注目すべき${signals[1]}件のシグナルを検出`,
    es: `IXMetrics encontró ${signals[1]} señales para seguir hoy`,
    ko: `IXMetrics가 오늘 주목할 신호 ${signals[1]}개를 찾았습니다`,
    'zh-TW': `IXMetrics 找到今天值得關注的 ${signals[1]} 個訊號`,
    'pt-BR': `O IXMetrics encontrou ${signals[1]} sinais para acompanhar hoje`,
    de: `IXMetrics hat heute ${signals[1]} beachtenswerte Signale gefunden`,
  });
  ...
  const addHandle = source.match(/^Add\s+(@[^\s]+)\s+on IXMetrics$/i);
  if (addHandle) return localized({ ja: `IXMetricsで${addHandle[1]}を追加`, es: `Añade a ${addHandle[1]} en IXMetrics`, ko: `IXMetrics에서 ${addHandle[1]} 추가`, 'zh-TW': `在 IXMetrics 新增 ${addHandle[1]}`, 'pt-BR': `Adicione ${addHandle[1]} no IXMetrics`, de: `${addHandle[1]} bei IXMetrics hinzufügen` });
  const profileTitle = source.match(/^(.+?)\s+on IXMetrics$/i);
  if (profileTitle) return localized({ ja: `IXMetricsの${profileTitle[1]}`, es: `${profileTitle[1]} en IXMetrics`, ko: `IXMetrics의 ${profileTitle[1]}`, 'zh-TW': `${profileTitle[1]} 的 IXMetrics`, 'pt-BR': `${profileTitle[1]} no IXMetrics`, de: `${profileTitle[1]} bei IXMetrics` });
  const scoutLevel = source.match(/^Analyst Level:\s*(.+)$/i);
  if (scoutLevel) return localized({ ja: `アナリストレベル：${scoutLevel[1]}`, es: `Nivel de analista: ${scoutLevel[1]}`, ko: `분석가 레벨: ${scoutLevel[1]}`, 'zh-TW': `分析師等級：${scoutLevel[1]}`, 'pt-BR': `Nível de analista: ${scoutLevel[1]}`, de: `Analystenstufe: ${scoutLevel[1]}` });
  ...
  const clearedLine = source.match(/^(.+?) cleared this line in (\d+) of its last (.+?) completed games\. IXMetrics also checks recent scoring plus verified starter and team context where available\.$/i);
  if (clearedLine) {
    const team = inline(clearedLine[1]);
    const sampleSize = inline(clearedLine[3]);
    return localized({ ja: `${team}は終了済み直近${sampleSize}試合のうち${clearedLine[2]}試合でこのラインを達成しました。IXMetricsは最近の得点状況と検証済みの先発・チーム情報も確認します。`, es: `${team} superó esta línea en ${clearedLine[2]} de sus últimos ${sampleSize} partidos finalizados. IXMetrics también revisa anotación reciente y contexto verificado de abridor y equipo.`, ko: `${team}은 최근 종료 ${sampleSize}경기 중 ${clearedLine[2]}경기에서 이 기준을 넘었습니다. IXMetrics는 최근 득점과 검증된 선발 및 팀 상황도 확인합니다.`, 'zh-TW': `${team} 在最近 ${sampleSize} 場已結束比賽中有 ${clearedLine[2]} 場達成此標準。IXMetrics 也會檢查近期得分及已驗證的先發與球隊情境。`, 'pt-BR': `${team} superou esta linha em ${clearedLine[2]} dos últimos ${sampleSize} jogos concluídos. O IXMetrics também verifica a pontuação recente e o contexto confirmado do titular e do time.`, de: `${team} erreichte diese Linie in ${clearedLine[2]} der letzten ${sampleSize} beendeten Spiele. IXMetrics prüft außerdem aktuelle Runs sowie verifizierten Starter- und Teamkontext.` });
  }
  ...
  const exactLine = source.match(/^(.+?) met this exact line in (\d+) of the last (.+?) completed games\. IXMetrics combines that recent result with season context; this is a support rating, not a guaranteed probability\.$/i);
  if (exactLine) {
    const sampleSize = inline(exactLine[3]);
    return localized({ ja: `${exactLine[1]}は終了済み直近${sampleSize}試合のうち${exactLine[2]}試合でこの条件を達成しました。IXMetricsは最近の結果とシーズン状況を組み合わせます。これは支持度であり、保証された確率ではありません。`, es: `${exactLine[1]} cumplió esta línea en ${exactLine[2]} de los últimos ${sampleSize} partidos. IXMetrics combina ese resultado con el contexto de temporada; es una valoración de respaldo, no una probabilidad garantizada.`, ko: `${exactLine[1]}은 최근 종료 ${sampleSize}경기 중 ${exactLine[2]}경기에서 이 기준을 달성했습니다. IXMetrics는 최근 결과와 시즌 상황을 결합합니다. 이는 지지도이며 보장 확률이 아닙니다.`, 'zh-TW': `${exactLine[1]} 在最近 ${sampleSize} 場已結束比賽中有 ${exactLine[2]} 場達成此條件。IXMetrics 會結合近期結果與賽季情境；這是支持度評分，不是保證機率。`, 'pt-BR': `${exactLine[1]} atingiu esta linha em ${exactLine[2]} dos últimos ${sampleSize} jogos. O IXMetrics combina esse resultado com o contexto da temporada; é uma avaliação de apoio, não uma probabilidade garantida.`, de: `${exactLine[1]} erfüllte diese Linie in ${exactLine[2]} der letzten ${sampleSize} Spiele. IXMetrics kombiniert das Ergebnis mit dem Saisonkontext; dies ist eine Unterstützungsbewertung, keine garantierte Wahrscheinlichkeit.` });
  }
  ...
  const hitterLine = source.match(/^(.+?) cleared this (.+?) line in (\d+) of the last (.+?) tracked games\. IXMetrics combines recent results with season production and verified opposing-starter context when available\.$/i);
  if (hitterLine) {
    const category = inline(hitterLine[2]);
    const sampleSize = inline(hitterLine[4]);
    return localized({ ja: `${hitterLine[1]}は追跡対象の直近${sampleSize}試合中${hitterLine[3]}試合で${category}の条件を達成しました。IXMetricsは最近の結果、シーズン成績、検証済みの相手先発情報を組み合わせます。`, es: `${hitterLine[1]} superó la línea de ${category} en ${hitterLine[3]} de los últimos ${sampleSize} partidos. IXMetrics combina resultados recientes, producción de temporada y contexto verificado del abridor rival.`, ko: `${hitterLine[1]}은 최근 추적 ${sampleSize}경기 중 ${hitterLine[3]}경기에서 ${category} 기준을 넘었습니다. IXMetrics는 최근 결과, 시즌 성적, 검증된 상대 선발 상황을 결합합니다.`, 'zh-TW': `${hitterLine[1]} 在最近 ${sampleSize} 場追蹤比賽中有 ${hitterLine[3]} 場達成 ${category} 標準。IXMetrics 結合近期結果、賽季表現與已驗證的對方先發情境。`, 'pt-BR': `${hitterLine[1]} superou a linha de ${category} em ${hitterLine[3]} dos últimos ${sampleSize} jogos. O IXMetrics combina resultados recentes, produção da temporada e contexto confirmado do titular adversário.`, de: `${hitterLine[1]} erreichte die ${category}-Linie in ${hitterLine[3]} der letzten ${sampleSize} Spiele. IXMetrics kombiniert aktuelle Ergebnisse, Saisonleistung und verifizierten Kontext des gegnerischen Starters.` });
  }
  ...
  const pitcherLine = source.match(/^(.+?) cleared this (.+?) line in (\d+) of the last (.+?) tracked pitching appearances\. IXMetrics adjusts the recent trend with workload and opponent context where verified\.$/i);
  if (pitcherLine) {
    const category = inline(pitcherLine[2]);
    const sampleSize = inline(pitcherLine[4]);
    return localized({ ja: `${pitcherLine[1]}は追跡対象の直近${sampleSize}登板中${pitcherLine[3]}登板で${category}の条件を達成しました。IXMetricsは最近の傾向を登板負荷と対戦状況で調整します。`, es: `${pitcherLine[1]} superó la línea de ${category} en ${pitcherLine[3]} de sus últimas ${sampleSize} apariciones. IXMetrics ajusta la tendencia con carga de trabajo y contexto del rival.`, ko: `${pitcherLine[1]}은 최근 추적 ${sampleSize}등판 중 ${pitcherLine[3]}등판에서 ${category} 기준을 넘었습니다. IXMetrics는 투구 부담과 상대 상황으로 최근 추세를 조정합니다.`, 'zh-TW': `${pitcherLine[1]} 在最近 ${sampleSize} 場追蹤登板中有 ${pitcherLine[3]} 場達成 ${category} 標準。IXMetrics 會依工作量與對手情境調整近期趨勢。`, 'pt-BR': `${pitcherLine[1]} superou a linha de ${category} em ${pitcherLine[3]} das últimas ${sampleSize} aparições. O IXMetrics ajusta a tendência com carga e contexto do adversário.`, de: `${pitcherLine[1]} erreichte die ${category}-Linie in ${pitcherLine[3]} der letzten ${sampleSize} Einsätze. IXMetrics passt den Trend anhand von Belastung und Gegnerkontext an.` });
  }
...
export const translateUiText = (value: string, locale: ScoutLocale) => {
  ...
  if (locale === 'en') return applyBrandCopy(value, locale);
  ...
  return applyBrandCopy(translated ? preserveOuterWhitespace(value, translated) : value, locale);
};
