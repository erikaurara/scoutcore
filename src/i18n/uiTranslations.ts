...
const translateDynamic = (source: string, locale: ScoutLocale): string | null => {
  ...
  if (batterFinalLine) return localized({ ja: `最終成績：${batterFinalLine[2]} ABで${batterFinalLine[1]}。打席：${batterFinalLine[3]}、四球：${batterFinalLine[4]}、三振：${batterFinalLine[5]}、塁打：${batterFinalLine[6]}。このレポートは終了済み試合の検証済みMLBボックススコア成績をまとめたものです。`, es: `Línea final: ${batterFinalLine[1]} en ${batterFinalLine[2]} AB. Apariciones: ${batterFinalLine[3]}, bases por bolas: ${batterFinalLine[4]}, ponches: ${batterFinalLine[5]}, bases totales: ${batterFinalLine[6]}. Este informe resume la producción verificada del box score de MLB.`, ko: `최종 기록: ${batterFinalLine[2]} AB에서 ${batterFinalLine[1]}. 타석: ${batterFinalLine[3]}, 볼넷: ${batterFinalLine[4]}, 삼진: ${batterFinalLine[5]}, 총 루타: ${batterFinalLine[6]}. 이 보고서는 종료된 경기의 검증된 MLB 박스스코어 성적을 요약합니다.`, 'zh-TW': `最終成績：${batterFinalLine[2]} AB 中 ${batterFinalLine[1]}。打席：${batterFinalLine[3]}、保送：${batterFinalLine[4]}、三振：${batterFinalLine[5]}、壘打數：${batterFinalLine[6]}。本報告彙整已結束比賽的已驗證 MLB 技術統計。`, 'pt-BR': `Linha final: ${batterFinalLine[1]} em ${batterFinalLine[2]} AB. Aparições: ${batterFinalLine[3]}, walks: ${batterFinalLine[4]}, strikeouts: ${batterFinalLine[5]}, bases totais: ${batterFinalLine[6]}. Este relatório resume a produção verificada do box score da MLB.`, de: `Endstand: ${batterFinalLine[1]} in ${batterFinalLine[2]} AB. Plate Appearances: ${batterFinalLine[3]}, Walks: ${batterFinalLine[4]}, Strikeouts: ${batterFinalLine[5]}, Total Bases: ${batterFinalLine[6]}. Dieser Bericht fasst verifizierte MLB-Boxscore-Leistungen zusammen.` });
