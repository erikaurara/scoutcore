...
export const PlayerPredictionsViewV2:React.FC=()=>{
 ...
 return <div className="min-h-screen bg-[#071225] px-4 py-5 text-[#edf4ff] sm:px-6 lg:px-8"><div className="mx-auto max-w-[1380px] space-y-4">
  ...
  </div>{filtersOpen&&<PredictionFilterPanel playerGroup={w.player?.group} playerTeamId={w.player?.currentTeam?.id??null} playerTeamName={w.player?.currentTeam?.name??null} opponentId={w.opponentId} opponents={w.opponents} selectedOpponent={w.selectedOpponent} pitcher={w.pitcher} withPlayer={w.withPlayer} withoutPlayer={w.withoutPlayer} pitcherHand={w.pitcherHand} homeAway={w.homeAway} setOpponent={w.setOpponentId} setPitcher={w.setPitcher} setWith={w.setWithPlayer} setWithout={w.setWithoutPlayer} setHand={w.setPitcherHand} setHomeAway={w.setHomeAway}/>} {w.selectedOpponent&&<p className="mt-2 text-[11px] text-[#59e8f3]">Opponent and pitcher filters use {w.selectedOpponent.name}. Teammate filters use {w.player?.currentTeam?.name??'the selected player’s team'}.</p>}</section>
  ...
  {w.error&&!w.loading&&<div className="flex flex-col gap-3 rounded-lg border border-[#ffcc66]/30 bg-[#ffcc66]/8 px-4 py-3 text-sm text-[#ffe0a3] sm:flex-row sm:items-center sm:justify-between"><span>{w.error}</span>{w.player&&w.logs.length>0&&<button type="button" onClick={w.clearFilters} className="rounded-lg border border-[#ffcc66]/45 px-3 py-2 text-xs font-black text-[#ffe0a3] hover:bg-[#ffcc66]/10">CLEAR FILTERS</button>}</div>}
