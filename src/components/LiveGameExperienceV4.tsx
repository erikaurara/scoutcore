...
export const LiveGameExperienceV4: React.FC<LiveGameExperienceProps> = ({ gamePk, feed, signedIn, userEmail, onOpenAuth }) => {
  ...
  const insight = useMemo(() => {
    ...
    if (pitcherKs >= 5) return `${playerName(pitcher, 'The pitcher')} has ${pitcherKs} strikeouts${pitcherIp ? ` through ${pitcherIp} innings` : ''}. IXMetrics is highlighting the current strikeout total from the verified box score.`;
    ...
    return latestDescription || 'IXMetrics will summarize the next verified MLB event when it reaches the live feed.';
    ...
    <main className="mx-auto w-full max-w-[1740px] px-3 py-4 sm:px-5">
      <section className="mb-3 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#2b405b] bg-[#0a1423] px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div><p className="text-[11px] font-black tracking-[.16em] text-[#00e6f4]">IXMETRICS AI LIVE SIM</p><p className="mt-0.5 text-[10px] text-[#718198]">Live MLB data · field alignment · pitch tracking · chat</p></div>
        </div>
      ...
      <div className="grid gap-3 xl:grid-cols-[280px_minmax(460px,1fr)_300px_340px]">
        ...
        <div className="space-y-3">
          ...
          <section className="overflow-hidden rounded-2xl border border-[#00e6f4]/25 bg-[#0d1727]">
            <div className="flex items-center gap-2 border-b border-[#26364e] px-4 py-3"><span className="material-symbols-outlined text-[18px] text-[#00e6f4]">graphic_eq</span><p className="text-[10px] font-black tracking-[.14em] text-white">IXMETRICS AI INSIGHT</p><span className="ml-auto rounded-full border border-[#65f2b5]/30 bg-[#65f2b5]/8 px-2 py-0.5 text-[8px] font-black text-[#65f2b5]">LIVE</span></div>
