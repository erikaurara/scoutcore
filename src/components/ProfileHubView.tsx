        ...
        <section className="overflow-hidden rounded-2xl border border-[#2a405b] bg-[#101a2d] shadow-[0_18px_55px_rgba(0,0,0,.22)]">
          ...
          <div className="px-5 pb-6 sm:px-8">
            <div className="-mt-14 flex items-end gap-4">
              ...
              <div className="min-w-0 pb-2"><div className="text-[10px] font-black uppercase tracking-[.17em] text-[#65f2b5]">ScoutCore profile</div><h1 className="mt-1 truncate text-2xl font-black text-white sm:text-3xl">{profile.display_name}</h1>{profile.username && <div className="mt-0.5 truncate text-sm font-bold text-[#50eaf4]">@{profile.username}</div>}</div>
            </div>
          ...
          <div className="mt-4 overflow-hidden rounded-2xl border border-[#2a405b] bg-[#0c1627]">
            {loadingPeople ? <div className="py-16 text-center text-sm text-[#8fa0b5]">Loading…</div> : requests.length ? requests.map((request) => (
              <div key={request.request_id} className="border-b border-[#263951] px-4 py-3 last:border-b-0">
                <button type="button" onClick={() => void openPublicProfile(request.profile_id, 'requests')} className="flex w-full items-center gap-3 text-left"><Avatar name={request.display_name} url={request.avatar_url} /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-bold text-white">{request.display_name}</span><span className="mt-0.5 block truncate text-xs text-[#50eaf4]">@{request.username || 'scout'}</span></span><span className="material-symbols-outlined text-[#71839b]">chevron_right</span></button>
      ...
      <section aria-busy={!profileReady} className="sc-profile-card relative overflow-hidden rounded-2xl border border-[#2a405b] bg-[radial-gradient(circle_at_10%_5%,rgba(0,240,255,.17),transparent_30%),linear-gradient(120deg,#0a1d31,#0b1728_58%,#101a2d)] shadow-[0_12px_36px_rgba(0,0,0,.18)]">
        ...
        <div aria-hidden={!profileReady} className={`sc-profile-card-body relative px-8 py-8 transition-opacity duration-150 ${profileReady ? 'opacity-100' : 'pointer-events-none select-none opacity-0'}`}>
          ...
          <div className="sc-profile-identity flex items-center gap-6 pr-20">
            ...
            <div className="min-w-0 flex-1"><div className="text-xs font-black uppercase tracking-[.18em] text-[#65f2b5]">ScoutCore Profile</div><h1 className="mt-1 truncate text-4xl font-black leading-tight text-white">{displayName}</h1>{username && <div className="mt-0.5 truncate text-sm font-bold text-[#50eaf4]">@{username}</div>}<div className="sc-profile-email mt-1 truncate text-xs text-[#8c9aae]">{userEmail}</div></div>
          </div>
