 ...
 return <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-xl border border-[#30415c] bg-[#0d182b] p-3.5">
  ...
  {player&&rows.length===0?<div className="mt-2 flex flex-1 items-center justify-center rounded-lg border border-dashed border-[#3a4d68] bg-[#091427]/75 px-4 text-center"><div><p className="text-base font-black text-white">No matching historical games</p><p className="mt-1 text-xs text-[#9fb0c5]">{emptyMessage??'Change or clear one of the filters.'}</p></div></div>:<>
   <div className="relative mt-2 h-[174px] shrink-0 pl-8 md:min-h-0 md:flex-1">
    <div className="absolute inset-y-4 left-0 z-10 flex w-6 flex-col justify-between bg-[#0d182b] text-right text-[10px] font-bold text-[#b8c6d8]"><span>{showValue(ceiling)}</span><span>{showValue(midpoint)}</span><span>0</span></div>
    ...
    {scrollable&&<span className="pointer-events-none absolute bottom-0 left-8 right-0 z-20 text-center text-[8px] font-semibold leading-[12px] tracking-wide text-[#6f8198]">SWIPE CHART TO VIEW ALL {rows.length} GAMES ↔</span>}
   </div>
   <div className="mt-0.5 grid h-[38px] shrink-0 grid-cols-5 divide-x divide-[#24344b] overflow-hidden rounded-md border border-[#263850] bg-[#091427]/80 text-center"><div className={statCell}><span className="block text-[7px] leading-none text-[#8395ad]">Hit Rate</span><b className="mt-1 block whitespace-nowrap text-[9px] leading-none text-[#59e8f3]">{success}/{rows.length} ({pct(filtered)})</b></div><div className={statCell}><span className="block text-[7px] leading-none text-[#8395ad]">Average</span><b className="mt-1 block max-w-full truncate text-[9px] leading-none text-white">{showSummaryValue(average)}</b></div><div className={statCell}><span className="block text-[7px] leading-none text-[#8395ad]">Median</span><b className="mt-1 block max-w-full truncate text-[9px] leading-none text-white">{showSummaryValue(median)}</b></div><div className={statCell}><span className="block text-[7px] leading-none text-[#8395ad]">Streak</span><b className="mt-1 block text-[9px] leading-none text-[#59e8f3]">{streak}</b></div><div className={statCell}><span className="block text-[7px] leading-none text-[#8395ad]">Sample</span><b className="mt-1 block whitespace-nowrap text-[9px] leading-none text-white">{rows.length} games</b></div></div>
