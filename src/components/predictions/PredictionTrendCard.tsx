...
export function PredictionTrendCard({player,rows,stat,target,recentRate,seasonRate,emptyMessage}:{player:PredictionPlayer|null;rows:PredictionRow[];stat?:PredictionStat;target?:PredictionTarget;recentRate:number;seasonRate:number;emptyMessage?:string}){
 ...
 const showSummaryValue=(value:number)=>stat?.display?stat.display(value):Number.isInteger(value)?String(value):value.toFixed(1);
 ...
 // Keep L20/L30/Season bars the same readable width as L10 instead of
 // squeezing every game into the viewport. Longer ranges scroll horizontally.
 const chartWidth=scrollable?rows.length*30:undefined;
 const statCell='flex min-w-0 flex-col items-center justify-center px-0.5';
 return <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-xl border border-[#30415c] bg-[#0d182b] p-3.5">
  ...
  {player&&rows.length===0?<div className="mt-2 flex flex-1 items-center justify-center rounded-lg border border-dashed border-[#3a4d68] bg-[#091427]/75 px-4 text-center"><div><p className="text-base font-black text-white">No matching historical games</p><p className="mt-1 text-xs text-[#9fb0c5]">{emptyMessage??'Change or clear one of the filters.'}</p></div></div>:<>
   <div className="relative mt-2 h-[174px] shrink-0 pl-8 md:min-h-0 md:flex-1">
    <div className="absolute inset-y-4 left-0 z-10 flex w-6 flex-col justify-between bg-[#0d182b] text-right text-[11px] font-bold text-[#c8d4e3]"><span>{showValue(ceiling)}</span><span>{showValue(midpoint)}</span><span>0</span></div>
    <div className={`h-full ${scrollable?'overflow-x-auto overscroll-x-contain [-webkit-overflow-scrolling:touch] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden':''}`} style={{touchAction:scrollable?'pan-x':'auto'}} onTouchStart={e=>{if(scrollable)e.stopPropagation()}} onTouchMove={e=>{if(scrollable)e.stopPropagation()}}>
     <div className="h-full" style={chartWidth?{width:chartWidth,minWidth:chartWidth}:undefined}>
      ...
      <div className="mt-1 grid gap-1.5 px-2 text-center text-[8px] leading-tight text-[#b0bfd1]" style={{gridTemplateColumns:`repeat(${Math.max(1,rows.length)},minmax(0,1fr))`}}>{rows.map((r,i)=><span key={`${r.gamePk}-label-${i}`} className="truncate">{(r as any).opponentAbbreviation??(r as any).opponentName??''}<br/>{(r as any).date?String((r as any).date).slice(5).replace('-','/'):''}</span>)}</div>
     </div>
    </div>
   </div>
   <div className="mt-0.5 grid h-[40px] max-h-[40px] min-h-[40px] shrink-0 grid-cols-5 divide-x divide-[#24344b] overflow-hidden rounded-md border border-[#263850] bg-[#091427]/80 text-center"><div className={statCell}><span className="block text-[8px] leading-none text-[#a5b5c9]">Hit Rate</span><b className="mt-1 block whitespace-nowrap text-[10px] leading-none text-[#59e8f3]">{success}/{rows.length} ({pct(filtered)})</b></div><div className={statCell}><span className="block text-[8px] leading-none text-[#a5b5c9]">Average</span><b className="mt-1 block max-w-full truncate text-[10px] leading-none text-white">{showSummaryValue(average)}</b></div><div className={statCell}><span className="block text-[8px] leading-none text-[#a5b5c9]">Median</span><b className="mt-1 block max-w-full truncate text-[10px] leading-none text-white">{showSummaryValue(median)}</b></div><div className={statCell}><span className="block text-[8px] leading-none text-[#a5b5c9]">Streak</span><b className="mt-1 block text-[10px] leading-none text-[#59e8f3]">{streak}</b></div><div className={statCell}><span className="block text-[8px] leading-none text-[#a5b5c9]">Sample</span><b className="mt-1 block whitespace-nowrap text-[10px] leading-none text-white">{rows.length} games</b></div></div>
   <div className="mt-1 grid h-[12px] shrink-0 grid-cols-2 gap-2 text-center text-[9px] leading-[12px]"><span className="text-white">L10 <b>{pct(recentRate)}</b></span><span className="text-white">Season <b>{pct(seasonRate)}</b></span></div>
  </>}
 </div>;
}
