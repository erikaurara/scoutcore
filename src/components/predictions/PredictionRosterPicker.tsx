...
export function PredictionRosterPicker({ label, value, onPick, teamId, teamName, group }: { label: string; value: PredictionPlayerChoice | null; onPick: (value: PredictionPlayerChoice | null) => void; teamId: number | null; teamName?: string | null; group?: 'hitting' | 'pitching' }) {
  ...
  return <div className="relative min-w-0">
    <div className="mb-1.5 flex items-center justify-between gap-2 text-[10px]"><span className="text-[#b8c6d8]">{label}</span>{teamName && <span className="truncate font-bold text-[#59e8f3]">{teamName} ONLY</span>}</div>
    ...
    {value && selectedName.isLocalized && <div className="mt-1 text-[10px] text-[#dce8f6]">{selectedName.displayName}<span className="ml-2 text-[#8fa0b7]">{selectedName.officialName}</span></div>}
