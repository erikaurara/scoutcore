...
export const PlayerPredictionsViewV3:React.FC=()=>{
 ...
 return <div className="bg-[#071225] px-3 py-2.5 text-[#edf4ff] md:h-[calc(100dvh-76px)] md:overflow-hidden"><div className="mx-auto flex max-w-[1500px] flex-col gap-2.5 md:h-full">
  ...
  <section className="rounded-xl border border-[#2b3f5b] bg-[#0d182b] p-2.5">
   <div className="grid grid-cols-1 gap-2 md:grid-cols-[1.15fr_.72fr_.58fr_2.25fr_auto_auto] md:items-end">
    <PredictionPlayerSearch value={w.player} onPick={pickPlayer}/>
    <label className="text-[10px] text-[#c2cede]">STAT<select value={w.statId} onChange={e=>w.setStatId(e.target.value)} className="mt-1 h-9 w-full rounded-lg border border-[#30415c] bg-[#091427] px-2.5 text-xs font-bold text-white [color-scheme:dark]">{w.statDefs.map(d=><option key={d.id} value={d.id}>{d.label}</option>)}</select></label>
    <label className="text-[10px] text-[#c2cede]">TARGET<select value={w.targetIndex} onChange={e=>w.setTargetIndex(Number(e.target.value))} className="mt-1 h-9 w-full rounded-lg border border-[#30415c] bg-[#091427] px-2.5 text-xs font-bold text-white [color-scheme:dark]">{w.stat?.targets.map((t,i)=><option key={t.label} value={i}>{t.label}</option>)}</select></label>
    <div><span className="mb-1 block text-[10px] text-[#c2cede]">WINDOW</span><div className="grid h-9 grid-cols-6 rounded-lg border border-[#30415c] bg-[#091427] p-0.5">{(['L5','L10','L20','L30','SEASON','H2H'] as PredictionWindow[]).map(x=><button key={x} type="button" onClick={()=>w.setWindowKey(x)} className={`min-w-0 rounded-md px-1 text-[10px] font-bold whitespace-nowrap ${w.windowKey===x?'bg-[#59e8f3] text-[#042f36]':'text-white hover:bg-[#14243a]'}`}>{x==='SEASON'?'Season':x}</button>)}</div></div>
    <button type="button" onClick={()=>setFiltersOpen(v=>!v)} className={`h-9 rounded-lg border px-2.5 text-[11px] font-bold ${filtersOpen?'border-[#00e6f4] text-[#00e6f4]':'border-[#30415c] text-white'}`}>FILTERS {filtersOpen?'⌃':'⌄'}</button>
    <button type="button" onClick={w.refresh} className="h-9 rounded-lg border border-[#00e6f4] px-2.5 text-[11px] font-bold text-[#00e6f4]">REFRESH</button>
   </div>
  ...
  {w.loading&&<div className="rounded-lg border border-[#59e8f3]/25 bg-[#59e8f3]/5 px-3 py-1.5 text-xs text-[#9deff5]">Loading MLB game logs…</div>}
  {w.error&&!w.loading&&<div className="rounded-lg border border-[#ffcc66]/30 bg-[#ffcc66]/8 px-3 py-1.5 text-xs text-[#ffe0a3]">{w.error}</div>}

  {noMatches?<section className="rounded-xl border border-[#30415c] bg-[#0d182b] p-3"><div className="flex items-center justify-between gap-4"><div><p className="text-[11px] font-bold text-[#56e9f4]">HISTORICAL TREND</p><h2 className="mt-0.5 text-lg font-bold text-white">No matching historical games</h2><p className="mt-0.5 text-xs text-[#9fb0c5]">{emptyMessage}</p></div><button type="button" onClick={w.clearFilters} className="rounded-lg border border-[#56e9f4]/55 px-3 py-1.5 text-[11px] font-black text-[#56e9f4]">CLEAR FILTERS</button></div></section>:<section className="grid min-h-0 flex-1 grid-cols-1 gap-2.5 md:grid-cols-[1.05fr_.95fr]"><PredictionTrendCard player={w.player} rows={w.rows} stat={w.stat} target={w.target} recentRate={w.recentRate} seasonRate={w.seasonRate} emptyMessage={emptyMessage}/><PredictionProjectionCard player={w.player} projection={w.projection} rows={w.rows.length} opponentName={w.selectedOpponent?.name} pitcherName={w.pitcher?.name}/></section>}
  ...
  {!noMatches&&w.player&&w.rows.length>0&&<>
   <section className="grid grid-cols-5 divide-x divide-[#2c3d56] rounded-xl border border-[#30415c] bg-[#0d182b] px-2 py-2 text-center">
    <div><p className="text-[10px] text-[#91a3bb]">AVG</p><b className="text-sm text-white">{fmt3(aggregate.avg)}</b></div>
    <div><p className="text-[10px] text-[#91a3bb]">OBP</p><b className="text-sm text-white">{fmt3(aggregate.obp)}</b></div>
    <div><p className="text-[10px] text-[#91a3bb]">vs {aggregate.hand==='L'?'LHP':'RHP'}</p><b className="text-sm text-white">{fmt3(aggregate.handAvg)}</b></div>
    <div><p className="text-[10px] text-[#91a3bb]">Last 10 AVG</p><b className="text-sm text-white">{fmt3(aggregate.last10Avg)}</b></div>
    <div><p className="text-[10px] text-[#91a3bb]">Opponent WHIP</p><b className="text-sm text-white">{pitcherWhip==null?'—':pitcherWhip.toFixed(2)}</b></div>
   </section>

   <section className="grid grid-cols-2 gap-2 md:grid-cols-5">{quickStats.map((def:PredictionStat)=><article key={def.id} className="rounded-lg border border-[#30415c] bg-[#0d182b] px-2.5 py-2"><h3 className="text-[12px] font-bold text-white">{def.label}</h3><div className="mt-1.5 grid gap-1.5" style={{gridTemplateColumns:`repeat(${Math.min(4,def.targets.length)},minmax(0,1fr))`}}>{def.targets.slice(0,4).map(target=>{const r=rateFor(def,target);return <div key={target.label} className="min-w-0 text-center"><div className="mx-auto flex h-7 w-full max-w-12 items-end overflow-hidden rounded bg-[#17243a]"><div className={`w-full ${r===0?'bg-[#ff515a]':'bg-[#59e8f3]'}`} style={{height:`${r===0?5:Math.max(5,Math.round(r*100))}%`}}/></div><b className={`block text-[11px] ${r===0?'text-[#ff6b73]':'text-white'}`}>{pct(r)}</b><span className="block truncate text-[9px] text-[#93a5bb]">{target.label}</span></div>})}</div></article>)}</section>
  </>}

  <section className="rounded-lg border border-[#2b3f5b] bg-[#0d182b] px-3 py-2"><div className="flex items-center gap-2"><span className="material-symbols-outlined text-[18px] text-[#59e8f3]">menu_book</span><div><h3 className="text-[12px] font-bold text-white">How to read this</h3><p className="text-[10px] text-[#9fb0c5]">Historical Trend shows past qualifying games. ScoutCore Projection estimates support for the selected outcome; it is not a guarantee.</p></div></div></section>
 </div></div>;
};
