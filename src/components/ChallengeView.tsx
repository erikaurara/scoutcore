...
async function analyzeHitterSelection(selection: PickSelection, game: MlbScheduleGame): Promise<PickAnalysis> {
  ...
  if (!recent.length && !Object.keys(season).length) return { chance: 'LIMITED DATA', score: 0, summary: 'IXMetrics does not have enough verified hitter data for this selection yet.', stats: [] };
  ...
  return {
    ...
    summary: `${selection.subjectName} cleared this ${category.shortLabel.toLowerCase()} line in ${successes} of the last ${recent.length || 'available'} tracked games. IXMetrics combines recent results with season production and verified opposing-starter context when available.`,
...
async function analyzePitcherSelection(selection: PickSelection, game: MlbScheduleGame): Promise<PickAnalysis> {
  ...
  if (!recent.length && !Object.keys(season).length) return { chance: 'LIMITED DATA', score: 0, summary: 'IXMetrics does not have enough verified pitcher data for this selection yet.', stats: [] };
  ...
  return {
    chance: chanceFromScore(score),
    score: Math.round(score),
    summary: `${selection.subjectName} cleared this ${category.shortLabel.toLowerCase()} line in ${successes} of the last ${recent.length || 'available'} tracked pitching appearances. IXMetrics adjusts the recent trend with workload and opponent context where verified.`,
...
async function analyzeGameSelection(selection: PickSelection, game: MlbScheduleGame): Promise<PickAnalysis> {
  ...
  if (selection.type === 'team_runs' || selection.type === 'team_hits' || selection.type === 'team_winner' || selection.type === 'game_first_team_score') {
    ...
    summary = `${selectedTeam?.name ?? 'The selected team'} cleared this line in ${successes} of its last ${total || 'available'} completed games. IXMetrics also checks recent scoring plus verified starter and team context where available.`;
    ...
    keyFactor = 'IXMetrics uses verified first-inning scoring history rather than a generic full-game scoring rate.';
    stats.push(
...
export const ChallengeView: React.FC<ChallengeViewProps> = ({ initialGame = null, initialTeamId = null, signedIn, userEmail, onOpenAuth, onLeaderboardBack }) => {
  ...
  const [displayName, setDisplayName] = useState('IXMetrics User');
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setDisplayName(metadata.display_name || metadata.full_name || user.email?.split('@')[0] || 'IXMetrics User');
  ...
  const lockPrediction = async () => {
    ...
    setMessage('Prediction locked ✓ IXMetrics saved the analysis that existed when you pressed GO.');
    setSelectedPicks([]);
    setAnalysis({});
    setTab('mine');
    setMyTab('upcoming');
  ...
  return <div className="min-h-screen bg-[#07101f] text-[#dae2fd] px-3 py-4 sm:px-5 lg:px-6 pb-28">
    <div className="mx-auto max-w-[1680px] space-y-5">
      <section className="rounded-2xl border border-[#233c58] bg-[radial-gradient(circle_at_12%_0%,rgba(0,240,255,.08),transparent_30%),#0c1728] px-5 py-5">
        <div className="flex flex-col gap-5 2xl:flex-row 2xl:items-center 2xl:justify-between">
          <div className="flex items-start gap-4">
            ...
            <div><p className="text-[10px] font-extrabold uppercase tracking-[.18em] text-[#65f2b5]">IX Challenge</p><h1 className="mt-1 text-3xl font-extrabold text-white sm:text-4xl">Build your predictions. Analyze before you lock.</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-[#9badc2]">Select baseball milestones, review IXMetrics' pick-specific analysis and verified supporting statistics, edit anything you want, then press GO to make the card official.</p></div>
          </div>
      ...
      {tab === 'build' && !signedIn && <section className="rounded-2xl border border-[#31506f] bg-[#0f182b] p-8 text-center"><span className="material-symbols-outlined text-5xl text-[#00e6f4]">fact_check</span><h2 className="mt-4 text-2xl font-bold text-white">Log in to build a IX Challenge</h2><p className="mx-auto mt-2 max-w-2xl text-sm leading-6 text-[#91a0b5]">Challenge cards are predictions only—no money, no odds. Your account saves the analysis that existed when you locked each pick.</p><button onClick={onOpenAuth} className="mt-5 rounded-xl bg-[#00e6f4] px-6 py-3 text-sm font-extrabold text-[#00363a]">LOG IN</button></section>}
      ...
      {tab === 'build' && signedIn && <>
        ...
        {selectedGame && <div className="grid gap-4 xl:grid-cols-[390px_minmax(0,1fr)_380px]">
          ...
          <aside id="challenge-summary" className="h-fit rounded-2xl border border-[#31506f] bg-[#0b1425] p-4 xl:sticky xl:top-20">
            ...
            <div className="mt-4 rounded-xl border border-[#00e6f4]/20 bg-[#00e6f4]/5 p-3 text-xs leading-5 text-[#a9dce1]"><span className="material-symbols-outlined mr-2 align-middle text-[18px] text-[#00e6f4]">verified_user</span>Your full IXMetrics analysis is saved with the prediction at the moment you press GO.</div>
      ...
      {tab === 'mine' && signedIn && <section>
        <div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-2xl font-bold text-white">My Picks</h2><p className="mt-1 text-sm text-[#91a0b5]">Compare what you predicted, what IXMetrics thought beforehand, and what actually happened.</p></div><button onClick={refreshResults} disabled={refreshingResults} className="rounded-xl border border-[#30415c] px-4 py-2.5 text-xs font-bold text-[#00e6f4] disabled:opacity-50">{refreshingResults ? 'CHECKING RESULTS…' : 'REFRESH RESULTS'}</button></div>
      ...
      {tab === 'leaderboard' && <section>
        ...
        <p className="mt-2 text-[10px] text-[#718090]">Incorrect picks never remove points; they affect accuracy only. IX Points have no cash value and cannot be bought, exchanged, transferred or withdrawn.</p>
...
const FlowSteps = ({ selected, analyzed }: { selected: boolean; analyzed: boolean }) => {
  const steps = [
    ...
    { label: 'ANALYZE', copy: 'IXMetrics runs analysis', active: selected },
