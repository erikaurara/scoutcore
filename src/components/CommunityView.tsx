  ...
  return <div className="min-h-screen bg-[#0b1326] text-[#dae2fd] px-3 py-5 sm:px-6 lg:px-8">
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <section className="rounded-2xl border border-[#2a405b] bg-[#101a2d] p-4 sm:p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_170px_150px] gap-3"><input value={title} onChange={e => setTitle(e.target.value)} maxLength={90} placeholder="Post title — e.g. Dodgers bullpen matchup tonight" className="h-11 rounded-lg border border-[#30415c] bg-[#0b1425] px-3 text-sm outline-none focus:border-[#00f0ff]"/><select value={category} onChange={e => setCategory(e.target.value as CommunityPost['category'])} className="h-11 rounded-lg border border-[#30415c] bg-[#0b1425] px-3 text-sm outline-none"><option>Game Thread</option><option>Analysis</option><option>Hot Take</option></select><input value={tag} onChange={e => setTag(e.target.value)} maxLength={24} placeholder="Team / player" className="h-11 rounded-lg border border-[#30415c] bg-[#0b1425] px-3 text-sm outline-none focus:border-[#00f0ff]"/></div>
