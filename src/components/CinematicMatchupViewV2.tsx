...
export const CinematicMatchupViewV2: React.FC = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setBatterPitchProfile(nextPitchProfile);
    });
  }, [batterId]);
  ...
  return <div className="min-h-screen bg-[#08111f] text-[#dae2fd] p-6 lg:p-8 space-y-5">
    ...
    {matchup && selectedBatter && <>
      <section className="grid grid-cols-1 xl:grid-cols-[1fr_230px_1fr] gap-5 items-stretch">
        ...
        <PlayerPanel type="batter" player={selectedBatter} profile={batterPitchProfile} splits={splits} />
      </section>
...
const PlayerPanel = ({ type, player, profile, splits }: any) => {
  const isPitcher = type === 'pitcher';
  const s = player.stats ?? {};
  const handLabel = isPitcher ? `${player.pitchHand ?? '?'}HP` : `${player.batSide ?? '?'}HB`;
  return <section className={`bg-[#151f33] rounded-lg border border-[#26364e] ${isPitcher ? 'border-l-2 border-l-[#54dce9]' : 'border-r-2 border-r-[#cbd7ef]'} p-5 min-h-[470px]`}>
    <div className="flex items-start gap-4">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-2 flex items-center gap-2 flex-wrap"><span className="font-display-lg text-lg font-normal tracking-wide text-[#54dce9]">{handLabel}</span>{!isPitcher && <span className="text-xs text-[#849495]">· {player.position ?? ''}</span>}</div>
      </div>
    </div>
    ...
    {isPitcher ? <>
      ...
      <div className="mt-6"><p className="font-label-caps text-[10px] text-[#54dce9]">PITCH ARSENAL</p><div className="mt-3 space-y-2">{profile?.slice(0, 5).map((p: any) => <div key={p.code} className="grid grid-cols-[120px_1fr_62px] gap-2 items-center text-xs"><span className="truncate">{p.name}</span><div className="h-2 bg-[#25344c] rounded-full overflow-hidden"><div className="h-full bg-[#54dce9] rounded-full" style={{ width: `${Math.max(10, Math.min(100, p.usagePct))}%` }} /></div><span className="text-right text-[#b9c8de]">{p.avgVelo?.toFixed?.(1) ?? '—'} mph</span></div>)}</div></div>
      <p className="mt-6 text-xs leading-relaxed text-[#aab8ce]">{pitcherScout(player, profile)}</p>
    </> : <>
      <div className="grid grid-cols-5 gap-2 mt-7"><Data label="AVG" value={s.avg} /><Data label="HR" value={s.homeRuns} /><Data label="RBI" value={s.rbi} /><Data label="SB" value={s.stolenBases} /><Data label="OPS" value={s.ops} /></div>
      <BatterPitchProfile profile={profile} />
      <p className="mt-6 text-xs leading-relaxed text-[#aab8ce]">{batterScout(player, splits, profile)}</p>
    </>}
  </section>;
};

const BatterPitchProfile = ({ profile }: any) => <div className="mt-6">
  <div className="flex items-center justify-between gap-3"><p className="font-label-caps text-[10px] text-[#54dce9]">PITCH-TYPE HITTING PROFILE</p><div className="grid grid-cols-2 gap-5 text-[9px] text-[#849495] font-label-caps"><span>AVG</span><span>SLG</span></div></div>
  {profile?.length ? <div className="mt-3 space-y-2">{profile.slice(0, 5).map((p: any) => {
    const avg = p.avg == null ? null : Number(p.avg);
    const slg = p.slg == null ? null : Number(p.slg);
    const strength = slg == null ? 18 : Math.max(12, Math.min(100, (slg / .700) * 100));
    return <div key={p.code} className="grid grid-cols-[120px_1fr_44px_44px] gap-2 items-center text-xs"><span className="truncate">{p.name}</span><div className="h-2 bg-[#25344c] rounded-full overflow-hidden"><div className="h-full bg-[#54dce9] rounded-full" style={{ width: `${strength}%` }} /></div><span className="text-right font-data-numeric">{avg == null ? '—' : avg.toFixed(3).replace(/^0/, '')}</span><span className="text-right font-data-numeric text-[#b9c8de]">{slg == null ? '—' : slg.toFixed(3).replace(/^0/, '')}</span></div>;
  })}</div> : <p className="mt-3 text-xs text-[#849495]">Recent pitch-type results are still building from MLB pitch-by-pitch data.</p>}
</div>;
...
const OtherBatters = ({ batters, selected, onChoose }: any) => <section className="bg-[#10192b] border border-[#26364e] rounded-xl overflow-hidden">
  <div className="px-4 py-3 border-b border-[#26364e]"><p className="font-label-caps text-[9px] text-[#54dce9]">OTHER BATTERS</p><h3 className="font-bold text-sm">Click another hitter to compare instantly</h3></div>
  <div className="flex gap-2 overflow-x-auto p-3">
    {batters.map((b: any) => <button key={b.id} onClick={() => onChoose(b.id)} className={`min-w-[185px] flex items-center gap-3 p-2.5 rounded-lg border text-left ${selected === b.id ? 'border-[#54dce9] bg-[#163348]' : 'border-[#26364e] bg-[#151f33] hover:border-[#54dce9]/50'}`}>
      <Headshot id={b.id} name={b.name} small />
      <div className="min-w-0 flex-1"><p className="font-bold text-xs truncate">{b.name}</p><p className="text-[9px]"><span className="text-[#54dce9]">{b.batSide ?? '?'}</span><span className="text-[#849495]"> · {b.position || '—'}</span></p></div>
...
const CombinedGameLog = ({ pitcher, batter, pitcherLogs, batterLogs }: any) => {
  ...
  return <section className="bg-[#10192b] border border-[#26364e] rounded-xl overflow-hidden">
    <div className="p-4 border-b border-[#26364e] flex flex-wrap items-center justify-between gap-3">
      <div><p className="font-label-caps text-[9px] text-[#54dce9]">RECENT GAME LOG</p><h3 className="font-bold text-sm">Last 10 games</h3></div>
...
const InjuredList = ({ matchup }: any) => <section className="bg-[#10192b] border border-[#26364e] rounded-xl overflow-hidden">
  <div className="p-4 border-b border-[#26364e]"><p className="font-label-caps text-[9px] text-[#54dce9]">INJURED LIST</p><h3 className="font-bold text-sm">{matchup.team.name}</h3></div>
...
const pitcherScout = (player: any, profile: any[]) => {
  const s = player.stats ?? {}; const top = profile?.[0];
  if (!top) return `${player.name} is a ${player.pitchHand === 'L' ? 'left-handed' : 'right-handed'} pitcher with a ${s.era ?? '—'} ERA, ${s.whip ?? '—'} WHIP and ${s.strikeoutsPer9Inn ?? '—'} K/9 in 2026.`;
...
const batterScout = (player: any, splits: any, profile: any[]) => {
  const leftOps = splits?.vsLeft?.ops; const rightOps = splits?.vsRight?.ops;
  const side = player.batSide === 'L' ? 'left-handed' : player.batSide === 'R' ? 'right-handed' : player.batSide === 'S' ? 'switch' : '';
  const best = [...(profile ?? [])].filter((p:any)=>p.slg != null).sort((a:any,b:any)=>Number(b.slg)-Number(a.slg))[0];
  const weakest = [...(profile ?? [])].filter((p:any)=>p.slg != null).sort((a:any,b:any)=>Number(a.slg)-Number(b.slg))[0];
  const splitText = leftOps && rightOps ? ` His 2026 OPS is ${leftOps} vs LHP and ${rightOps} vs RHP.` : '';
  const pitchText = best ? ` In recent tracked plate appearances, his strongest pitch-type production has come against ${best.name} (${Number(best.slg).toFixed(3).replace(/^0/,'')} SLG).${weakest && weakest.code !== best.code ? ` His lowest tracked SLG in this sample is against ${weakest.name} (${Number(weakest.slg).toFixed(3).replace(/^0/,'')}).` : ''}` : '';
  return `${player.name} is a ${side} hitter.${splitText}${pitchText} These pitch-type reads use recent MLB pitch-by-pitch results and should be treated as a current-sample scouting view.`;
};

const calcAdvantage = (pitcher: any, batter: any, splits: any) => {
  const pitcherEra = Number(pitcher?.stats?.era || 4.2); const pitcherWhip = Number(pitcher?.stats?.whip || 1.3); const batterOps = Number(batter?.stats?.ops || .700); const split = pitcher?.pitchHand === 'L' ? Number(splits?.vsLeft?.ops || batterOps) : Number(splits?.vsRight?.ops || batterOps); const raw = 50 + (4.2 - pitcherEra) * 3 + (1.3 - pitcherWhip) * 8 + (.720 - split) * 25; return Math.round(Math.max(28, Math.min(72, raw)));
};
