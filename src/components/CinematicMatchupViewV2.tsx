...
export const CinematicMatchupViewV2: React.FC = () => {
  ...
  return <div className="min-h-screen bg-[#08111f] text-[#dae2fd] p-6 lg:p-8 space-y-5">
    ...
    {matchup && selectedBatter && <>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-[1.35fr_.9fr] gap-5 items-start"><VerticalBatters matchup={matchup} selected={batterId} onChoose={setBatterId} /><PitcherContext pitcher={matchup.pitcher} pitcherLogs={pitcherLogs} batters={matchup.batters} /></section>
...
const PitcherContext = ({ pitcher, pitcherLogs, batters }: any) => {
  const s = pitcher?.stats ?? {};
  const rows = (pitcherLogs ?? []).slice(0,3);
  const avgOps = avgNumber((batters ?? []).map((b:any) => Number(b.stats?.ops)));
  const totalHr = (batters ?? []).reduce((sum:number,b:any) => sum + (Number(b.stats?.homeRuns) || 0), 0);
  const totalSo = (batters ?? []).reduce((sum:number,b:any) => sum + (Number(b.stats?.strikeOuts) || 0), 0);
