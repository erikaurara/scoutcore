  ...
  return <div className="min-h-screen bg-[#0b1326] text-[#dae2fd] p-8 space-y-6">
    ...
    {analytics && away && home && <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      {[away, home].map((team: any) => <div key={team.side} className="bg-[#171f33] rounded-xl border border-[#3b494b]/20 p-6">
        ...
        <div className="mt-6 flex items-center gap-4"><div className="w-16 h-16 rounded-lg overflow-hidden bg-[#222a3d] border border-[#3b494b]/30">{team.pitcher?.id && <img src={mlbPlayerHeadshotUrl(team.pitcher.id, 160)} alt={team.pitcher.name} className="w-full h-full object-cover object-top" />}</div><div><p className="text-[10px] text-[#849495]">STARTING PITCHER</p><p className="font-bold text-lg">{team.pitcher?.name ?? 'TBD'}</p><p className="text-xs text-[#849495]">ERA {team.pitcher?.stats?.era ?? '—'} · WHIP {team.pitcher?.stats?.whip ?? '—'} · K/9 {team.pitcher?.stats?.strikeoutsPer9Inn ?? '—'}</p></div></div>
        ...
        <div className="mt-6 space-y-2">{team.matchups?.slice(0,5).map((m: any) => <div key={m.batter.id} className="flex items-center justify-between text-xs bg-[#131b2e] rounded-lg p-2"><div className="flex items-center gap-2 min-w-0"><img src={mlbPlayerHeadshotUrl(m.batter.id, 96)} alt={m.batter.name} className="w-8 h-8 rounded-md object-cover object-top bg-[#222a3d]" /><span className="truncate">{m.batter.name}</span></div><b className="text-[#65f2b5]">{m.analysis?.score ?? '—'}</b></div>)}</div>
      </div>)}
