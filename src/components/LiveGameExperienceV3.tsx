...
export const LiveGameExperienceV3: React.FC<LiveGameExperienceProps> = ({ gamePk, feed, signedIn, userEmail, onOpenAuth }) => {
  ...
  return (
    <main className="mx-auto max-w-[1740px] px-3 py-4 sm:px-5">
      ...
      <div className="grid gap-3 xl:grid-cols-[270px_minmax(430px,1fr)_300px_330px]">
        ...
        <div className="space-y-3">
          <section className="overflow-hidden rounded-2xl border border-[#2b405b] bg-[#0d1727]">
            <div className="border-b border-[#26364e] px-4 py-3"><p className="text-[10px] font-black tracking-[.14em] text-white">LINESCORE</p></div>
        ...
        <aside className="flex min-h-[690px] flex-col overflow-hidden rounded-2xl border border-[#2b405b] bg-[#0d1727] xl:max-h-[820px]">
          <div className="flex items-center justify-between gap-3 border-b border-[#26364e] px-4 py-3">
            <div><p className="text-sm font-black text-white">LIVE GAME CHAT</p><p className="mt-1 text-[10px] text-[#8fa0b7]">Tap a profile picture to view the Scout and follow them.</p></div>
          ...
          <div className="flex-1 space-y-3 overflow-y-auto px-4 py-4">
            {messages.length ? messages.map((message) => {
              ...
              const profileId = social?.profile_id || (message.user_id !== 'preview-user' ? message.user_id : null);
              const target: SocialProfileTarget = { profileId, displayName: shownName, avatarUrl: social?.avatar_url || null };
              return <div key={message.id} className="rounded-xl border border-[#26364e] bg-[#10192b] p-3">
                <div className="flex items-start gap-2.5">
                  <button type="button" onClick={() => setSelectedSocial(target)} className="shrink-0 rounded-full outline-none ring-[#00e6f4] focus-visible:ring-2" aria-label={`Open ${shownName} profile`}><SocialAvatar displayName={shownName} avatarUrl={social?.avatar_url || null} size="sm" /></button>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-2">
                      <button type="button" onClick={() => setSelectedSocial(target)} className="truncate text-left text-[11px] font-bold text-[#00e6f4] hover:underline">{shownName}</button>
                      <span className="shrink-0 text-[9px] text-[#607086]">{new Date(message.created_at).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}</span>
                    </div>
                    <p className="mt-1.5 whitespace-pre-wrap break-words text-sm leading-5 text-[#d7e0ee]">{message.body}</p>
                  </div>
                </div>
              </div>;
      ...
      <SocialProfileCard target={selectedSocial} signedIn={signedIn} onOpenAuth={onOpenAuth} onClose={() => setSelectedSocial(null)} onFollowChanged={() => void loadChatSocial()} />
    </main>
  );
};
