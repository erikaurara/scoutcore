...
function mapGame(game: any): MlbScheduleGame {
  ...
  return { gamePk: game.gamePk, gameDate: game.gameDate, status: game.status?.abstractGameState ?? 'Unknown', detailedState: game.status?.detailedState ?? 'Unknown', awayTeam: { id: team('away').id, name: team('away').name ?? 'Away Team', abbreviation: team('away').abbreviation }, homeTeam: { id: team('home').id, name: team('home').name ?? 'Home Team', abbreviation: team('home').abbreviation }, awayScore: game.teams?.away?.score, homeScore: game.teams?.home?.score, awayProbablePitcher: probable('away')?.id ? { id: probable('away').id, name: probable('away').fullName ?? 'TBD' } : undefined, homeProbablePitcher: probable('home')?.id ? { id: probable('home').id, name: probable('home').fullName ?? 'TBD' } : undefined };
}
...
export const ScheduleView: React.FC = () => {
  ...
  return <div className="min-h-screen bg-[#0b1326] text-[#dae2fd] p-6 space-y-5">
    <section className="flex flex-col xl:flex-row xl:items-end justify-between gap-4 border-b border-[#3b494b]/20 pb-5">
      <div className="flex items-center gap-3">
        ...
        <div><span className="font-label-caps text-[10px] text-[#65f2b5]">LIVE MLB DATA</span><h1 className="font-display-lg text-3xl text-[#dbfcff]">MLB Schedule</h1><p className="text-sm text-[#9aa9aa] mt-0.5">Daily games and results in ScoutCore's own design.</p></div>
      </div>
      <div className="flex items-center gap-4 text-xs font-semibold"><span className="flex items-center gap-2"><i className="w-2.5 h-2.5 rounded-full bg-[#ff5c68]"/>Live</span><span className="flex items-center gap-2"><i className="w-2.5 h-2.5 rounded-full bg-[#00f0ff]"/>Final</span></div>
    </section>
    ...
    <section className="relative bg-[#131b2e] border border-[#3b494b]/20 rounded-xl p-3">
      <div className="flex items-center max-w-max">
        ...
        <button onClick={() => { setCalendarMonth(new Date(`${date}T12:00:00Z`)); setCalendarOpen(v => !v); }} className="px-5 py-2.5 bg-[#171f33] border-y border-[#3b494b]/30 text-sm font-bold hover:text-[#00f0ff] min-w-[250px]">{displayDate(date)} <span className="material-symbols-outlined align-middle text-base ml-1">expand_more</span></button>
    ...
    {loading ? <div className="p-8 text-center text-[#9aa9aa] text-sm bg-[#171f33] rounded-xl">Loading MLB schedule…</div> : games.length === 0 ? <div className="p-8 text-center text-[#9aa9aa] text-sm bg-[#171f33] rounded-xl">No MLB games are scheduled for this date.</div> : <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-3">{games.map((game) => <GameCard key={game.gamePk} game={game} />)}</div>}
    <div className="text-xs text-[#849495] flex gap-4"><span>All game times shown in Eastern Time (ET).</span><span>Scores refresh automatically.</span></div>
  </div>;
};
...
const MonthCalendar = ({ month, selectedDate, onChangeMonth, onSelect }: { month: Date; selectedDate: string; onChangeMonth: (d: Date) => void; onSelect: (key: string) => void }) => {
  const year = month.getUTCFullYear(); const monthIndex = month.getUTCMonth(); const first = new Date(Date.UTC(year, monthIndex, 1, 12)); const daysInMonth = new Date(Date.UTC(year, monthIndex + 1, 0, 12)).getUTCDate(); const leading = first.getUTCDay(); const cells = Array.from({ length: leading + daysInMonth }, (_, i) => i < leading ? null : i - leading + 1); const moveMonth = (amount: number) => onChangeMonth(new Date(Date.UTC(year, monthIndex + amount, 1, 12))); const today = toDateKey(new Date());
...
const GameCard = ({ game }: { game: MlbScheduleGame }) => {
  const isFinal = game.detailedState === 'Final' || game.status === 'Final'; const isLive = game.status === 'Live'; const status = isFinal ? 'FINAL' : isLive ? 'LIVE' : game.detailedState === 'Postponed' ? 'POSTPONED' : gameTime(game);
  return <div className="bg-[#131b2e] border border-[#3b494b]/25 rounded-xl overflow-hidden hover:border-[#00f0ff]/40 transition-colors">
    <div className="px-3 py-2.5 border-b border-[#3b494b]/20 flex items-center justify-between"><span className={`flex items-center gap-2 font-label-caps text-xs font-bold ${isLive ? 'text-[#ff5c68]' : isFinal ? 'text-[#00f0ff]' : 'text-[#c7d2d3]'}`}>{(isLive || isFinal) && <i className={`w-2.5 h-2.5 rounded-full ${isLive ? 'bg-[#ff5c68]' : 'bg-[#00f0ff]'}`}/>} {status}</span><span className="material-symbols-outlined text-[#849495] text-lg">star</span></div>
    <div className="p-3 space-y-3"><TeamRow team={game.awayTeam} score={game.awayScore} /><TeamRow team={game.homeTeam} score={game.homeScore} /><div className="pt-2.5 border-t border-[#3b494b]/20"><p className="text-[10px] text-[#849495] font-label-caps">PROBABLE STARTERS</p><p className="text-xs text-white mt-1 truncate">{game.awayProbablePitcher?.name ?? 'TBD'} <span className="text-[#849495]">vs</span> {game.homeProbablePitcher?.name ?? 'TBD'}</p></div></div>
  </div>;
};

const TeamRow = ({ team, score }: { team: MlbScheduleGame['awayTeam']; score?: number }) => <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-lg bg-white/95 p-1.5 flex items-center justify-center"><img src={mlbTeamLogoUrl(team.id)} alt={`${team.name} logo`} className="max-w-full max-h-full object-contain" /></div><div className="min-w-0 flex-1"><p className="font-bold text-sm truncate">{team.name}</p><p className="text-[10px] text-[#849495]">{team.abbreviation ?? ''}</p></div><span className="font-data-numeric text-2xl text-[#dbfcff]">{score ?? '—'}</span></div>;
