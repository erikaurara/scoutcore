...
export const ScheduleView: React.FC<ScheduleViewProps> = ({ onOpenGame, onOpenTeam }) => {
  ...
  useEffect(() => { const id = window.setInterval(() => void load(), 5 * 60 * 1000); return () => window.clearInterval(id); }, [date]);

  useEffect(() => {
    let localIds: number[] = [];
    try { const raw = window.localStorage.getItem(localFollowKey); const parsed = raw ? JSON.parse(raw) : []; if (Array.isArray(parsed)) localIds = parsed.map(Number).filter(Number.isFinite); } catch {}
    const applyLocal = () => setFollowedGames(new Set(localIds));
    if (!supabase) { applyLocal(); return; }
    supabase.auth.getUser().then(({ data }) => {
      const user = data.user ?? null;
      setAccountUser(user);
      const metadata = user?.user_metadata ?? {};
      const saved = Array.isArray(metadata.followed_games) ? metadata.followed_games.map(Number).filter(Number.isFinite) : [];
  ...
  return <div className="min-h-screen bg-[#0b1326] text-[#dae2fd] px-3 py-3 sm:p-6 space-y-3 sm:space-y-5">
    <section className="flex items-center justify-between gap-3 border-b border-[#3b494b]/20 pb-3 sm:pb-5">
      <div className="min-w-0">
        <span className="font-label-caps text-[9px] sm:text-[10px] text-[#65f2b5]">LIVE MLB DATA</span>
        <h1 className="font-display-lg text-2xl sm:text-3xl leading-tight text-[#dbfcff]">MLB Schedule</h1>
        <p className="hidden sm:block text-sm text-[#9aa9aa] mt-0.5">Your favorite team appears first. Open any game or jump directly to a team.</p>
      </div>
      <div className="flex items-center shrink-0">
        <div className="flex rounded-lg border border-[#30415c] overflow-hidden">
          {(['ET','Local'] as TimeMode[]).map(mode => <button key={mode} onClick={() => setTimeMode(mode)} className={`px-3 py-2 text-[10px] font-bold ${timeMode === mode ? 'bg-[#00f0ff] text-[#002c31]' : 'bg-[#111a2d] text-[#9ba9b7]'}`}>{mode === 'ET' ? 'ET' : 'LOCAL'}</button>)}
        </div>
      </div>
    </section>

    <section className="relative">
      <div className="flex items-stretch w-full rounded-xl overflow-hidden border border-[#3b494b]/25 bg-[#131b2e]">
        <button onClick={() => setDate(shiftDate(date, -1))} className="w-12 shrink-0 flex items-center justify-center bg-[#171f33] hover:text-[#00f0ff]" aria-label="Previous day"><span className="material-symbols-outlined text-xl">chevron_left</span></button>
        <button onClick={() => { setCalendarMonth(new Date(`${date}T12:00:00Z`)); setCalendarOpen(v => !v); }} className="flex-1 min-w-0 px-2 py-3 bg-[#131b2e] text-xs sm:text-sm font-bold hover:text-[#00f0ff] truncate">{displayDate(date)} <span className="material-symbols-outlined align-middle text-base ml-1">expand_more</span></button>
        <button onClick={() => setDate(shiftDate(date, 1))} className="w-12 shrink-0 flex items-center justify-center bg-[#171f33] hover:text-[#00f0ff]" aria-label="Next day"><span className="material-symbols-outlined text-xl">chevron_right</span></button>
      </div>
    ...
    <div className="flex items-center justify-between gap-3 px-0.5"><h2 className="text-sm sm:text-xl font-bold truncate">{displayDate(date)}</h2><span className="text-[9px] text-[#849495] shrink-0">{updatedAt ? `UPDATED ${updatedAt.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}` : ''}</span></div>
    ...
    {loading ? <div className="p-8 text-center text-[#9aa9aa] text-sm bg-[#171f33] rounded-xl">Loading MLB schedule…</div>
      ...
      : <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-2.5 sm:gap-3">{sortedGames.map((game) => <GameCard key={game.gamePk} game={game} timeMode={timeMode} followed={followedGames.has(game.gamePk)} favoriteTeamId={favoriteTeamId} onOpen={() => onOpenGame(game)} onToggleFollow={() => void toggleFollow(game.gamePk)} onOpenTeam={onOpenTeam} />)}</div>}

    <div className="hidden sm:flex text-xs text-[#849495] flex-wrap gap-4"><span>{timeMode === 'ET' ? 'Game times shown in Eastern Time (ET).' : `Game times shown in your local timezone (${localZone}).`}</span><span>Scores refresh automatically.</span>{favoriteTeamId && <span className="text-[#7df4ff]">● Cyan dot = your favorite team.</span>}<span className="text-[#d6b96a]">★ Gold star = a game you follow.</span></div>
  </div>;
};
...
const MonthCalendar = ({ month, selectedDate, onChangeMonth, onSelect }: { month: Date; selectedDate: string; onChangeMonth: (d: Date) => void; onSelect: (key: string) => void }) => {
  ...
  return <div className="absolute z-50 top-[58px] left-0 w-[340px] max-w-full bg-[#10192b] border border-[#30415c] rounded-xl shadow-2xl p-3">
...
const GameCard = ({ game, timeMode, followed, favoriteTeamId, onOpen, onToggleFollow, onOpenTeam }: { game: MlbScheduleGame; timeMode: TimeMode; followed: boolean; favoriteTeamId: number | null; onOpen: () => void; onToggleFollow: () => void; onOpenTeam: (teamId: number) => void }) => {
  ...
  const openFromKeyboard = (event: React.KeyboardEvent<HTMLDivElement>) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onOpen(); } };
  return <div role="button" tabIndex={0} onClick={onOpen} onKeyDown={openFromKeyboard} className={`group cursor-pointer rounded-xl overflow-hidden focus:outline-none focus:border-[#00f0ff] transition-all ${isFavoriteTeamGame ? 'bg-[#122033] border border-[#00f0ff]/45' : 'bg-[#131b2e] border border-[#3b494b]/25 hover:border-[#00f0ff]/55'}`}>
    <div className="px-3 py-2 border-b border-[#3b494b]/20 flex items-center justify-between gap-2">
      <span className={`flex items-center gap-1.5 font-label-caps text-[10px] sm:text-xs font-bold ${isLive ? 'text-[#ff5c68]' : isFinal ? 'text-[#00f0ff]' : 'text-[#c7d2d3]'}`}>{(isLive || isFinal) && <i className={`w-2 h-2 rounded-full ${isLive ? 'bg-[#ff5c68]' : 'bg-[#00f0ff]'}`}/>} {status}</span>
      {!isFavoriteTeamGame && <button type="button" aria-label={followed ? 'Unfollow this game' : 'Follow this game'} aria-pressed={followed} onClick={(event) => { event.stopPropagation(); onToggleFollow(); }} className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${followed ? 'text-[#e7c96f]' : 'text-[#849495] hover:text-[#e7c96f]'}`}><span className="material-symbols-outlined text-lg" style={{ fontVariationSettings: followed ? "'FILL' 1" : "'FILL' 0" }}>star</span></button>}
    </div>
    <div className="px-3 py-2.5 space-y-1.5">
      ...
      <div className="pt-2 mt-1 border-t border-[#3b494b]/20 flex items-center gap-2 min-w-0"><span className="text-[8px] sm:text-[9px] text-[#849495] font-label-caps shrink-0">STARTERS</span><p className="text-[10px] sm:text-xs text-white truncate">{game.awayProbablePitcher?.name ?? 'TBD'} <span className="text-[#849495]">vs</span> {game.homeProbablePitcher?.name ?? 'TBD'}</p></div>
    </div>
  </div>;
};

const TeamRow = ({ team, score, favorite, onOpen }: { team: MlbScheduleGame['awayTeam']; score?: number; favorite: boolean; onOpen: () => void }) => <button type="button" onClick={(event) => { event.stopPropagation(); onOpen(); }} className={`w-full flex items-center gap-2 rounded-lg py-0.5 text-left transition-colors group/team ${favorite ? 'bg-[#00f0ff]/5' : ''}`} title={`Open ${team.name} team profile`}><div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-[#e7ebf0] p-1 flex items-center justify-center shrink-0"><img src={mlbTeamLogoUrl(team.id)} alt={`${team.name} logo`} className="max-w-full max-h-full object-contain" /></div><div className="min-w-0 flex-1"><div className="flex items-center gap-1.5 min-w-0"><p className={`font-bold text-[13px] sm:text-sm truncate group-hover/team:text-[#00f0ff] ${favorite ? 'text-[#e9fdff]' : ''}`}>{team.name}</p>{favorite && <span className="w-1.5 h-1.5 shrink-0 rounded-full bg-[#00f0ff]" />}</div><p className={`text-[9px] ${favorite ? 'text-[#7df4ff]' : 'text-[#849495]'}`}>{team.abbreviation ?? ''}</p></div><span className="font-data-numeric text-xl sm:text-2xl text-[#dbfcff]">{score ?? '—'}</span></button>;
