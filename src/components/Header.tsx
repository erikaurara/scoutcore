...
export const Header: React.FC<HeaderProps> = ({ currentTab, onOpenReport, onBack, onOpenMobileNav }) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 lg:left-72 h-16 bg-[#0b1326]/90 backdrop-blur-xl z-40 border-b border-[#3b494b]/20 flex items-center justify-between px-3 sm:px-4 lg:px-8 select-none">
      <div className="flex items-center gap-2 min-w-0">
        <button onClick={onOpenMobileNav} aria-label="Open menu" className="w-9 h-9 rounded-lg border border-[#31405b] bg-[#111a2d] text-[#b9cacb] hover:text-[#00f0ff] lg:hidden flex items-center justify-center shrink-0"><span className="material-symbols-outlined text-[21px]">menu</span></button>
        <div className="lg:hidden flex items-center gap-2 min-w-0">
          <img src={LOGO_URL} alt="ScoutCore" className="h-7 w-auto" />
          <span className="font-headline-lg font-bold text-[#dbfcff] truncate">ScoutCore</span>
        </div>
      ...
      <div className="flex items-center gap-1.5 sm:gap-3 lg:gap-4">
        {isProfilePage && onBack && (
          <button onClick={onBack} aria-label="Go back" className="w-9 h-9 rounded-lg border border-[#31405b] bg-[#111a2d] text-[#b9cacb] hover:text-[#62ddeb] lg:hidden flex items-center justify-center"><span className="material-symbols-outlined text-[18px]">arrow_back</span></button>
        )}
        {showAiScoutReport && (
          <button onClick={onOpenReport} className="px-2 sm:px-3 py-1.5 rounded-lg bg-[#00f0ff]/10 hover:bg-[#00f0ff]/20 text-[#00f0ff] border border-[#00f0ff]/30 text-xs font-label-caps flex items-center gap-1.5 transition-all shadow-[0_0_10px_rgba(0,240,255,0.1)]"><span className="material-symbols-outlined text-[16px]">smart_toy</span><span className="hidden sm:inline">AI SCOUT REPORT</span><span className="sm:hidden">AI</span></button>
        )}
        <div className="hidden sm:block h-6 w-[1px] bg-[#3b494b]/30 mx-1" />
