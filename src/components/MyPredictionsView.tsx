...
type UpcomingFilter = 'all' | 'editable' | 'locked';
type FinishedFilter = 'all' | 'correct' | 'incorrect' | 'void';
...
const marketLabel=(s:Selection)=>String(s.label||s.market_label||s.market||s.pick_label||titleCase(String(s.type||'IXMetrics Pick')));
const subjectLabel=(s:Selection)=>String(s.subjectName||s.player_name||s.playerName||s.pitcher_name||s.hitter_name||s.subject_name||s.teamName||s.team_name||'IXMetrics Pick');
...
const cardIsEditable=(card:Card,now:number)=>card.status==='upcoming'&&Number.isFinite(cardDate(card).getTime())&&cardDate(card).getTime()>now;
const finishedCardOutcome=(card:Card):Exclude<FinishedFilter,'all'>=>{const settled=(card.selections||[]).filter(selection=>selection.result==='correct'||selection.result==='incorrect'||selection.result==='void');if(settled.length&&settled.every(selection=>selection.result==='void'))return'void';if(settled.some(selection=>selection.result==='incorrect'))return'incorrect';return'correct';};
const selectionPoints=(selection:Selection)=>Number(selection.points_earned??selection.pointsEarned??selection.points??0);
const cardPoints=(card:Card)=>Number(card.points_earned??card.pointsEarned??card.total_points??(card.selections||[]).reduce((sum,selection)=>sum+selectionPoints(selection),0));
...
const PickDetail=({s,finished}:{s:Selection;finished?:boolean})=>{
  ...
  return <div className="border-t border-[#263951] px-4 py-4 sm:px-5">
    ...
    <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-[#9aabc0]">
      ...
      {p>0&&<span>IXMetrics when picked: <b className="text-[#59e8f3]">{Math.round(p<=1?p*100:p)}%</b></span>}
    </div>
...
const GamePredictionCard=({card,finished,now,onUpdated,onOpenSummary}:{card:Card;finished?:boolean;now:number;onUpdated?:(card:Card)=>void;onOpenSummary?:()=>void})=>{
  ...
  const editable=!finished&&cardIsEditable(card,now);
  ...
  const save=async()=>{
    ...
    if(!supabase||!card.id){setMessage('IXMetrics could not save this card. Please refresh and try again.');return;}
    setSaving(true);setMessage(null);
    const{data,error}=await supabase.rpc('update_upcoming_challenge_card',{p_card_id:card.id,p_selections:draft});
    setSaving(false);
    if(error){const locked=/lock|start|finish/i.test(error.message||'');setMessage(locked?'This game has started, so its predictions are locked.':'IXMetrics could not save these changes yet. Please try again.');return;}
  ...
  return <section className="overflow-hidden rounded-2xl border border-[#2a405b] bg-[#101a2d]">
    ...
    {finished&&onOpenSummary&&<div className="border-t border-[#22354d] bg-[#0b1527] p-3"><button type="button" onClick={onOpenSummary} className="flex w-full items-center justify-center gap-2 rounded-xl border border-[#59e8f3]/65 px-4 py-2.5 text-[11px] font-black text-[#59e8f3]"><span className="material-symbols-outlined text-[17px]">summarize</span>VIEW GAME SUMMARY<span className="material-symbols-outlined text-[17px]">chevron_right</span></button></div>}
  </section>;
};

const UpcomingFilterBar=({value,onChange,counts}:{value:UpcomingFilter;onChange:(value:UpcomingFilter)=>void;counts:Record<UpcomingFilter,number>})=><section className="rounded-2xl border border-[#2a405b] bg-[#101a2d] p-3"><div className="flex items-center justify-between gap-3 px-1"><div><h2 className="text-base font-extrabold text-white">Upcoming Games</h2><p className="mt-0.5 text-[10px] text-[#8fa0b5]">Edit unlocked predictions before first pitch.</p></div><span className="rounded-lg border border-[#30415c] px-2.5 py-1 text-[10px] font-bold text-[#a7b5c6]">{counts.all} cards</span></div><div className="mt-3 grid grid-cols-3 gap-2">{(['all','editable','locked'] as UpcomingFilter[]).map(filter=><button key={filter} type="button" onClick={()=>onChange(filter)} className={`rounded-xl border px-2 py-2.5 text-[10px] font-extrabold sm:text-xs ${value===filter?'border-[#59e8f3] bg-[#59e8f3] text-[#062029]':'border-[#30415c] bg-[#0b1527] text-[#9aabc0]'}`}>{filter==='all'?'All':filter==='editable'?'Editable':'Locked'} ({counts[filter]})</button>)}</div></section>;

const FinishedFilterBar=({value,onChange,counts}:{value:FinishedFilter;onChange:(value:FinishedFilter)=>void;counts:Record<FinishedFilter,number>})=><section className="rounded-2xl border border-[#2a405b] bg-[#101a2d] p-3"><div className="flex items-center justify-between gap-3 px-1"><div><h2 className="text-base font-extrabold text-white">Finished Games</h2><p className="mt-0.5 text-[10px] text-[#8fa0b5]">Review settled picks, outcomes and points.</p></div><span className="rounded-lg border border-[#30415c] px-2.5 py-1 text-[10px] font-bold text-[#a7b5c6]">{counts.all} cards</span></div><div className="mt-3 grid grid-cols-4 gap-1.5">{(['all','correct','incorrect','void'] as FinishedFilter[]).map(filter=><button key={filter} type="button" onClick={()=>onChange(filter)} className={`min-w-0 rounded-xl border px-1 py-2.5 text-[9px] font-extrabold sm:text-xs ${value===filter?'border-[#59e8f3] bg-[#59e8f3] text-[#062029]':'border-[#30415c] bg-[#0b1527] text-[#9aabc0]'}`}>{filter==='all'?'All':filter==='correct'?'Correct':filter==='incorrect'?'Missed':'Void'}<span className="ml-1">{counts[filter]}</span></button>)}</div></section>;

const UpcomingHelpCard=()=> <section className="flex items-start gap-3 rounded-2xl border border-[#2a405b] bg-[#101a2d] p-4"><span className="material-symbols-outlined mt-0.5 text-[#59e8f3]">info</span><div><h3 className="text-sm font-extrabold text-white">How Upcoming works</h3><p className="mt-1 text-xs leading-5 text-[#8fa0b5]">Open a card to review picks. Use <b className="text-[#59e8f3]">Edit Predictions</b> while its countdown is active. IXMetrics locks changes automatically at first pitch.</p><p className="mt-1 text-[10px] text-[#718198]">Free predictions only—no money or prizes.</p></div></section>;

const FinishedSummaryView=({card,onBack}:{card:Card;onBack:()=>void})=>{
  const selections=(card.selections||[]).filter(selection=>selection.result==='correct'||selection.result==='incorrect'||selection.result==='void');
  const correct=selections.filter(selection=>selection.result==='correct').length;
  const missed=selections.filter(selection=>selection.result==='incorrect').length;
  const voided=selections.filter(selection=>selection.result==='void').length;
  const settled=correct+missed;
  const accuracy=pct(correct,settled);
  const {away,home}=matchupTeams(card);
  const date=cardDate(card);
  const awayScore=card.away_score??card.awayScore;
  const homeScore=card.home_score??card.homeScore;
  const hasScore=Number.isFinite(Number(awayScore))&&Number.isFinite(Number(homeScore));
  return <div className="min-h-screen bg-[#081225] px-4 py-5 text-[#dae2fd] sm:px-6 lg:px-8"><div className="mx-auto max-w-3xl"><header className="flex items-center gap-3"><button type="button" onClick={onBack} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-[#2d4059] bg-[#101a2d] text-white" aria-label="Back to finished predictions"><span className="material-symbols-outlined">arrow_back</span></button><div><p className="text-[10px] font-black uppercase tracking-[.15em] text-[#65f2b5]">Finished Predictions</p><h1 className="text-2xl font-black text-white sm:text-3xl">Game Summary</h1><p className="mt-0.5 text-xs text-[#8fa0b5]">{new Intl.DateTimeFormat('en',{weekday:'short',month:'short',day:'numeric'}).format(date)}</p></div></header>
    <section className="mt-5 overflow-hidden rounded-2xl border border-[#2a405b] bg-[#101a2d]"><div className="bg-[#0d1728] px-4 py-3 text-center text-[10px] font-black uppercase tracking-[.13em] text-[#55d87a]">Final</div><div className="grid grid-cols-[1fr_auto_1fr] items-center gap-4 px-4 py-6 sm:px-6"><div className="min-w-0 text-center"><p className="truncate text-lg font-black text-white">{away}</p>{hasScore&&<p className="mt-2 text-3xl font-black text-white">{Number(awayScore)}</p>}</div><span className="text-xs font-black uppercase tracking-[.12em] text-[#72869f]">{hasScore?'—':'vs'}</span><div className="min-w-0 text-center"><p className="truncate text-lg font-black text-white">{home}</p>{hasScore&&<p className="mt-2 text-3xl font-black text-white">{Number(homeScore)}</p>}</div></div><div className="grid grid-cols-4 border-t border-[#263951] bg-[#0b1527]"><div className="border-r border-[#263951] p-3 text-center"><p className="text-[8px] font-bold uppercase text-[#8394aa]">Accuracy</p><p className="mt-1 text-lg font-black text-[#59e8f3]">{accuracy}%</p></div><div className="border-r border-[#263951] p-3 text-center"><p className="text-[8px] font-bold uppercase text-[#8394aa]">Correct</p><p className="mt-1 text-lg font-black text-[#55d87a]">{correct}</p></div><div className="border-r border-[#263951] p-3 text-center"><p className="text-[8px] font-bold uppercase text-[#8394aa]">Missed</p><p className="mt-1 text-lg font-black text-[#ff747c]">{missed}</p></div><div className="p-3 text-center"><p className="text-[8px] font-bold uppercase text-[#8394aa]">Points</p><p className="mt-1 text-lg font-black text-[#f0b14f]">+{cardPoints(card)}</p></div></div></section>
    <section className="mt-4 rounded-2xl border border-[#2a405b] bg-[#101a2d] p-4 sm:p-5"><div className="flex items-center justify-between gap-3"><h2 className="text-lg font-extrabold text-white">Prediction Results</h2>{voided>0&&<span className="rounded-lg border border-[#526275] px-2 py-1 text-[10px] text-[#a8b6c8]">{voided} void</span>}</div><div className="mt-4 overflow-hidden rounded-xl border border-[#263951]">{selections.map((selection,index)=>{const good=selection.result==='correct';const isVoid=selection.result==='void';return <article key={selection.id||index} className="border-b border-[#263951] bg-[#0b1527] p-4 last:border-b-0"><div className="flex items-start gap-3"><span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border font-black ${good?'border-[#55d87a] text-[#55d87a]':isVoid?'border-[#718198] text-[#a8b6c8]':'border-[#ff5d6a] text-[#ff5d6a]'}`}>{good?'✓':isVoid?'—':'×'}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-extrabold text-white">{subjectLabel(selection)}</p><p className="mt-0.5 text-xs text-[#9aabc0]">{marketLabel(selection)}</p><p className="mt-2 text-[10px] text-[#718198]">Result: {resultLabel(selection)}</p></div><div className="shrink-0 text-right"><p className={`text-[10px] font-extrabold uppercase ${good?'text-[#55d87a]':isVoid?'text-[#a8b6c8]':'text-[#ff747c]'}`}>{good?'Correct':isVoid?'Void':'Missed'}</p>{selectionPoints(selection)>0&&<p className="mt-1 text-[10px] font-bold text-[#f0b14f]">+{selectionPoints(selection)} pts</p>}</div></div></article>})}</div></section>
    <p className="mt-4 flex items-center justify-center gap-2 rounded-xl border border-[#263951] px-4 py-3 text-center text-[10px] text-[#8191a6]"><span className="material-symbols-outlined text-[16px]">shield</span>Results track free IXMetrics predictions—no money or prizes.</p>
  </div></div>;
};
...
export const MyPredictionsView:React.FC<{onBack:()=>void}>=({onBack})=>{
  ...
  const upcoming=useMemo(()=>cards.filter(c=>c.status==='upcoming'||(c.selections||[]).some(s=>!s.result||s.result==='pending')).sort((a,b)=>cardDate(a).getTime()-cardDate(b).getTime()),[cards]);
  const finished=useMemo(()=>cards.filter(c=>c.status==='finished'||(c.selections||[]).some(s=>s.result==='correct'||s.result==='incorrect'||s.result==='void')).sort((a,b)=>cardDate(b).getTime()-cardDate(a).getTime()),[cards]);
  const upcomingCounts=useMemo<Record<UpcomingFilter,number>>(()=>({all:upcoming.length,editable:upcoming.filter(card=>cardIsEditable(card,now)).length,locked:upcoming.filter(card=>!cardIsEditable(card,now)).length}),[upcoming,now]);
  const visibleUpcoming=useMemo(()=>upcoming.filter(card=>upcomingFilter==='all'||(upcomingFilter==='editable'?cardIsEditable(card,now):!cardIsEditable(card,now))),[upcoming,upcomingFilter,now]);
  const finishedCounts=useMemo<Record<FinishedFilter,number>>(()=>({all:finished.length,correct:finished.filter(card=>finishedCardOutcome(card)==='correct').length,incorrect:finished.filter(card=>finishedCardOutcome(card)==='incorrect').length,void:finished.filter(card=>finishedCardOutcome(card)==='void').length}),[finished]);
  const visibleFinished=useMemo(()=>finished.filter(card=>finishedFilter==='all'||finishedCardOutcome(card)===finishedFilter),[finished,finishedFilter]);
  ...
  if(selectedFinishedCard)return <FinishedSummaryView card={selectedFinishedCard} onBack={()=>setSelectedFinishedCard(null)}/>;
