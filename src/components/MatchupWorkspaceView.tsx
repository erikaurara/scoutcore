...
const Avatar = ({ id, name, small = false }: { id: number; name: string; small?: boolean }) => { const [failed, setFailed] = useState(false); const size = small ? 'w-10 h-10 rounded-md' : 'w-28 h-28 rounded-xl'; return <div className={`${size} bg-[#222a3d] overflow-hidden shrink-0 flex items-center justify-center relative border border-[#3b494b]/25`}>{!failed ? <img src={mlbPlayerHeadshotUrl(id, small ? 96 : 240)} alt={name} className="absolute inset-0 w-full h-full object-cover object-top" onError={() => setFailed(true)} /> : <span className="text-[#00f0ff] text-xs font-bold">{playerInitials(name)}</span>}</div>; };
