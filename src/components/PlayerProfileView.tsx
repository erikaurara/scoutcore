...
export const PlayerProfileView: React.FC<Props> = ({ playerId, onOpenTeam }) => {
  ...
  return <div className="px-3 pt-1 pb-0.5 sm:px-8 sm:py-8 max-w-[1280px] mx-auto h-[calc(100svh-24px)] sm:h-auto overflow-hidden sm:overflow-visible flex flex-col sm:block gap-1.5 sm:space-y-6">
    ...
    <section className="sm:hidden shrink-0 bg-[#10192c] border border-[#00f0ff]/25 rounded-xl p-1.5">
      <div className="flex rounded-lg border border-[#30415c] overflow-hidden mb-1"><button type="button" onClick={()=>setMobileStatsView('season')} className={`flex-1 px-3 py-1 text-[11px] font-bold ${mobileStatsView==='season'?'bg-[#56d3df] text-[#07101f]':'bg-[#111a2d] text-[#9fb0c8]'}`}>{currentSeason()}</button><button type="button" onClick={()=>setMobileStatsView('career')} className={`flex-1 px-3 py-1 text-[11px] font-bold ${mobileStatsView==='career'?'bg-[#56d3df] text-[#07101f]':'bg-[#111a2d] text-[#9fb0c8]'}`}>Career</button></div>
      <div className="grid grid-cols-3 gap-1">{mobileCards.map(([l,v]:any)=><Stat key={l} l={l} v={v}/>)}</div>
    </section>
...
const Stat=({l,v}:{l:string;v:any})=><div className="bg-[#171f33] rounded-lg p-1 sm:p-3"><div className="text-[9px] sm:text-[10px] text-[#849495] leading-none">{l}</div><div className="font-mono text-sm sm:text-lg mt-1">{v??'—'}</div></div>;
