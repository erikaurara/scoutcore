...
export const PlayerProfileView: React.FC<Props> = ({ playerId, onOpenTeam }) => {
  ...
  return <div className="px-3 py-4 sm:px-8 sm:py-8 max-w-[1280px] mx-auto space-y-3 sm:space-y-6">
    ...
    <section className="sm:hidden bg-[#10192c] border border-[#00f0ff]/25 rounded-xl p-3">
      <div className="flex rounded-lg border border-[#30415c] overflow-hidden mb-3"><button type="button" onClick={()=>setMobileStatsView('season')} className={`flex-1 px-3 py-2 text-xs font-bold ${mobileStatsView==='season'?'bg-[#56d3df] text-[#07101f]':'bg-[#111a2d] text-[#9fb0c8]'}`}>{currentSeason()}</button><button type="button" onClick={()=>setMobileStatsView('career')} className={`flex-1 px-3 py-2 text-xs font-bold ${mobileStatsView==='career'?'bg-[#56d3df] text-[#07101f]':'bg-[#111a2d] text-[#9fb0c8]'}`}>Career</button></div>
    ...
    <section className="bg-[#171f33] border border-[#3b494b]/30 rounded-xl sm:rounded-2xl overflow-hidden">
      <div className="p-3 sm:p-5 border-b border-[#3b494b]/25"><div className="text-[9px] sm:text-[10px] text-[#00f0ff] uppercase tracking-wider">Recent Game Log</div><h2 className="text-base sm:text-xl font-semibold">Last 30 games</h2></div>
      <div className="max-h-[255px] sm:max-h-[420px] overflow-y-auto overscroll-contain">
        <table className="w-full table-fixed text-[9px] sm:text-sm"><thead className="sticky top-0 z-10 bg-[#10192c] text-[#849495]"><tr><th className="text-left px-2 py-2 sm:p-3 w-[14%] sm:w-auto">DATE</th><th className="text-left px-1 py-2 sm:p-3 w-[12%] sm:w-auto">OPP</th>{data.group==='pitching'?<><th>IP</th><th>H</th><th>ER</th><th>BB</th><th>SO</th></>:<><th>AB</th><th>R</th><th>H</th><th>HR</th><th>RBI</th><th>BB</th><th>SO</th><th>SB</th></>}</tr></thead><tbody>{data.logs.map((g:any,i:number)=><tr key={`${g.date}-${i}`} className="border-t border-[#3b494b]/20"><td className="px-2 py-2.5 sm:p-3 whitespace-nowrap"><span className="sm:hidden">{shortDate(g.date)}</span><span className="hidden sm:inline">{g.date}</span></td><td className="px-1 py-2.5 sm:p-3 font-semibold"><span className="sm:hidden">{shortTeam(g.opponent)}</span><span className="hidden sm:inline">{g.opponent}</span></td>{data.group==='pitching'?<><td className="text-center">{g.stat.inningsPitched??'—'}</td><td className="text-center">{g.stat.hits??'—'}</td><td className="text-center">{g.stat.earnedRuns??'—'}</td><td className="text-center">{g.stat.baseOnBalls??'—'}</td><td className="text-center">{g.stat.strikeOuts??'—'}</td></>:<><td className="text-center">{g.stat.atBats??'—'}</td><td className="text-center">{g.stat.runs??'—'}</td><td className="text-center">{g.stat.hits??'—'}</td><td className="text-center">{g.stat.homeRuns??'—'}</td><td className="text-center">{g.stat.rbi??'—'}</td><td className="text-center">{g.stat.baseOnBalls??'—'}</td><td className="text-center">{g.stat.strikeOuts??'—'}</td><td className="text-center">{g.stat.stolenBases??'—'}</td></>}</tr>)}</tbody></table>
      </div>
    </section>
  </div>;
};
