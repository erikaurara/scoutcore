...
export const PlayerProfileView: React.FC<Props> = ({ playerId, onOpenTeam }) => {
  ...
  return <div className="px-3 pt-1.5 pb-2 sm:px-8 sm:py-8 max-w-[1280px] mx-auto h-[calc(100svh-104px)] sm:h-auto overflow-hidden sm:overflow-visible flex flex-col sm:block gap-2 sm:space-y-6">
    ...
    <section className="sm:hidden shrink-0 bg-[#10192c] border border-[#00f0ff]/25 rounded-xl p-2">
      <div className="flex rounded-lg border border-[#30415c] overflow-hidden mb-1.5"><button type="button" onClick={()=>setMobileStatsView('season')} className={`flex-1 px-3 py-1.5 text-[11px] font-bold ${mobileStatsView==='season'?'bg-[#56d3df] text-[#07101f]':'bg-[#111a2d] text-[#9fb0c8]'}`}>{currentSeason()}</button><button type="button" onClick={()=>setMobileStatsView('career')} className={`flex-1 px-3 py-1.5 text-[11px] font-bold ${mobileStatsView==='career'?'bg-[#56d3df] text-[#07101f]':'bg-[#111a2d] text-[#9fb0c8]'}`}>Career</button></div>
    ...
    <section className="min-h-0 flex-1 sm:flex-none bg-[#171f33] border border-[#3b494b]/30 rounded-xl sm:rounded-2xl overflow-hidden flex flex-col">
      ...
      <div className="sm:hidden -mt-px min-h-0 flex-1 overflow-y-auto overscroll-contain bg-[#171f33] border-t border-[#3b494b]/25">
...
const Stat=({l,v}:{l:string;v:any})=><div className="bg-[#171f33] rounded-lg p-1.5 sm:p-3"><div className="text-[9px] sm:text-[10px] text-[#849495] leading-none">{l}</div><div className="font-mono text-sm sm:text-lg mt-1">{v??'—'}</div></div>;
