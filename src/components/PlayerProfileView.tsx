...
const pitchPositions = [{left:'18%',top:'24%'},{left:'63%',top:'17%'},{left:'39%',top:'60%'},{left:'76%',top:'57%'},{left:'13%',top:'68%'},{left:'52%',top:'38%'}];

export const PlayerProfileView: React.FC<Props> = ({ playerId, onOpenTeam }) => {
  const [data,setData]=useState<any>(null); const [loading,setLoading]=useState(false); const [pitchProfile,setPitchProfile]=useState<any[]>([]); const [pitchLoading,setPitchLoading]=useState(false);
  ...
  const cards=useMemo(()=>{if(!data)return[];const s=data.season??{};return data.group==='pitching'?[['G',s.gamesPlayed],['W-L',`${s.wins??0}-${s.losses??0}`],['ERA',s.era],['IP',s.inningsPitched],['SO',s.strikeOuts],['WHIP',s.whip]]:[['G',s.gamesPlayed],['AVG',s.avg],['HR',s.homeRuns],['RBI',s.rbi],['H',s.hits],['OPS',s.ops]];},[data]);
  ...
  return <div className="px-3 py-4 sm:px-8 sm:py-8 max-w-[1280px] mx-auto space-y-3 sm:space-y-6">
    ...
    <section className="bg-[#10192c] border border-[#00f0ff]/25 rounded-xl sm:rounded-2xl p-3 sm:p-5"><div className="text-[9px] sm:text-xs text-[#00f0ff] uppercase tracking-wider mb-2 sm:mb-3">Career Regular Season</div><div className="grid grid-cols-3 md:grid-cols-6 gap-2 sm:gap-3 text-sm">{data.group==='pitching'?<><Stat l="G" v={c.gamesPlayed}/><Stat l="W-L" v={`${c.wins??0}-${c.losses??0}`}/><Stat l="ERA" v={c.era}/><Stat l="IP" v={c.inningsPitched}/><Stat l="SO" v={c.strikeOuts}/><Stat l="WHIP" v={c.whip}/></>:<><Stat l="G" v={c.gamesPlayed}/><Stat l="AVG" v={c.avg}/><Stat l="HR" v={c.homeRuns}/><Stat l="RBI" v={c.rbi}/><Stat l="H" v={c.hits}/><Stat l="OPS" v={c.ops}/></>}</div></section>

    <section><div className="text-[9px] sm:text-xs text-[#00f0ff] uppercase tracking-wider mb-2 sm:mb-3">{currentSeason()} Regular Season</div><div className="grid grid-cols-3 md:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3">{cards.map(([l,v]:any)=><div key={l} className="bg-[#171f33] border border-[#3b494b]/25 rounded-lg sm:rounded-xl p-2.5 sm:p-4"><div className="text-[9px] sm:text-[10px] text-[#849495]">{l}</div><div className="text-lg sm:text-2xl font-mono mt-1 sm:mt-2">{v??'—'}</div></div>)}</div></section>
