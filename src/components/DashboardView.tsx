...
const PitcherMini = ({ pitcher }: { pitcher?: { id: number; name: string } }) => <div className="flex items-center gap-2"><div className="w-9 h-9 rounded-full bg-[#222a3d] overflow-hidden">{pitcher?.id && <img src={mlbPlayerHeadshotUrl(pitcher.id,96)} alt={pitcher.name} className="w-full h-full object-cover"/>}</div><span className="text-[10px] text-[#00f0ff] truncate">{pitcher?.name ?? 'TBD'}</span></div>;
