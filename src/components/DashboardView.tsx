    ...
    <div className="p-8 space-y-8">
      <section className="bg-[#131b2e] rounded-2xl border border-[#00f0ff]/20 overflow-hidden shadow-[0_0_30px_rgba(0,240,255,.04)]">
        ...
        <div className="p-6">
          ...
          {report?.signals?.length ? <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">{report.signals.slice(0,6).map((s,i)=><div key={i} className="rounded-xl bg-[#171f33] border border-[#3b494b]/15 p-4"><div className="flex justify-between"><span className="text-[#00f0ff] text-xs font-bold">{s.player || s.team || 'ScoutCore signal'}</span><span className="text-[#65f2b5] text-xs">{s.score ?? '—'}</span></div><p className="text-xs text-[#b9cacb] mt-2 leading-5">{s.reason}</p></div>)}</div> : <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">{featuredGames.map(game => <BriefGame key={game.gamePk} game={game} onClick={() => onSelectTab('matchups')} />)}{!featuredGames.length && <div className="col-span-full rounded-xl bg-[#171f33] p-5 text-sm text-[#9ba9b7]">Today’s game cards will appear here as soon as MLB schedule data is available.</div>}</div>}
