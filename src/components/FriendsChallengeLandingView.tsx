...
export const FriendsChallengeLandingView: React.FC<Props> = ({ onBack }) => {
  ...
  return <div className="min-h-screen bg-[radial-gradient(circle_at_50%_5%,rgba(0,229,244,.08),transparent_28%),linear-gradient(180deg,#06101f_0%,#081426_100%)] px-6 pb-12 pt-8 text-[#d9e4f5]"><div className="mx-auto max-w-6xl">
    ...
    {tab === 'play' && <section className="mt-5 rounded-2xl border border-[#29425e] bg-[#08172a]/92 p-5"><div className="py-3 text-center"><div className="text-xl font-black uppercase tracking-[.13em] text-[#45e8f0]">🏆 Choose a Game Mode</div><p className="mt-1 text-sm text-[#a1adbe]">Play with friends. Make predictions. Win together.</p></div><div className="mt-3 grid gap-4 lg:grid-cols-3">{cards.map(card => <article key={card.id} className="flex min-h-[260px] flex-col rounded-2xl border border-[#29425e] bg-[#0a182b] p-5"><div className={`flex h-16 w-16 items-center justify-center rounded-full border ${card.id === 'team_up' ? 'border-[#b88b12] text-[#f5bd18]' : 'border-[#2d687e] text-[#39e5ef]'}`}><span className="material-symbols-outlined text-[32px]">{card.icon}</span></div><div className="mt-4 flex items-center gap-2"><h2 className="text-lg font-black uppercase text-white">{card.title}</h2><span className={`rounded-md border px-2 py-0.5 text-[10px] font-black ${card.id === 'team_up' ? 'border-[#c49615] text-[#f5bd18]' : 'border-[#17cdd7] text-[#34e8f0]'}`}>{card.badge}</span></div><p className="mt-2 text-sm leading-5 text-[#9aa8bb]">{card.description}</p><p className={`mt-auto pt-4 text-[11px] font-black tracking-wide ${card.id === 'team_up' ? 'text-[#f5bd18]' : 'text-[#31e5ee]'}`}>★ {card.footer}</p><button onClick={() => void openFriendPicker(card.id)} className="mt-4 rounded-lg border border-[#23e5ef] bg-[#062031] px-4 py-3 text-sm font-black text-[#32e8f0]">START</button></article>)}</div>

    <div className="mt-5 rounded-2xl border border-[#29425e] bg-[#09172a] px-6 py-5"><div className="grid grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] items-center gap-4 text-center"><div><span className="material-symbols-outlined text-[34px] text-[#39e5ef]">person_add</span><div className="mt-2 text-xs font-black text-[#39e5ef]">INVITE FRIENDS</div></div><span className="material-symbols-outlined text-[#8fa0b5]">arrow_forward</span><div><span className="material-symbols-outlined text-[34px] text-[#be65ef]">lock</span><div className="mt-2 text-xs font-black text-[#be65ef]">PRIVATE PICKS</div></div><span className="material-symbols-outlined text-[#8fa0b5]">arrow_forward</span><div><span className="material-symbols-outlined text-[34px] text-[#55e8ad]">emoji_events</span><div className="mt-2 text-xs font-black text-[#55e8ad]">AUTO RESULTS</div></div><span className="material-symbols-outlined text-[#8fa0b5]">arrow_forward</span><div><span className="material-symbols-outlined text-[34px] text-[#f5bd18]">calendar_month</span><div className="mt-2 text-xs font-black text-[#f5bd18]">HISTORY</div></div></div><div className="mt-4 text-center text-xs text-[#8794a6]">🛡 All modes are free. No tickets required.</div></div></section>}
