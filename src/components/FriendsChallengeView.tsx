...
const modes: { id: Mode; icon: string; title: string; sub: string }[] = [
  { id: 'weekly_h2h', icon: '⚔️', title: 'Weekly Head-to-Head', sub: 'Your normal weekly predictions compete automatically.' },
  { id: 'same_game', icon: '⚾', title: 'Same Game: You vs Friend', sub: 'Choose the same MLB game, make private picks, then battle.' },
  { id: 'team_up', icon: '🤝', title: 'Team Up', sub: 'Choose the same MLB game and see how many picks you match.' },
];
...
export const FriendsChallengeView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const send = async (mode?: Mode, friend = selected) => { if (!supabase || !friend) return; setBusy('send'); const { error } = await supabase.rpc('create_friend_challenge', { p_profile_id: friend.profile_id, p_mode: mode ?? null }); if (!error) { setSelected(null); setView('active'); await load(); } setBusy(null); };
  ...
  const randomizeMode = async (id: string) => { if (!supabase) return; setBusy(id); await supabase.rpc('randomize_friend_challenge_mode', { p_challenge_id: id }); await load(); setBusy(null); };
  const chooseGame = async (id: string, game: MlbScheduleGame) => { if (!supabase) return; setBusy(id); await supabase.rpc('choose_friend_challenge_game', { p_challenge_id: id, p_game: gamePayload(game) }); await load(); setBusy(null); };
  const resetGames = async (id: string) => { if (!supabase) return; setBusy(id); await supabase.rpc('reset_friend_challenge_game_choices', { p_challenge_id: id }); await load(); setBusy(null); };
  const randomGame = async (id: string) => { if (!supabase) return; setBusy(id); await supabase.rpc('randomize_friend_challenge_game', { p_challenge_id: id }); await load(); setBusy(null); };
  ...
  return <div className="min-h-screen bg-[#081225] px-4 py-5 text-[#dae2fd] sm:px-6 lg:px-8"><div className="mx-auto max-w-5xl">
    ...
    {loading ? <div className="py-20 text-center text-sm text-[#8fa0b5]">Loading Friends Challenge…</div> : <div className="mt-5">
      ...
      {view === 'active' && <div className="space-y-4">{active.length ? active.map(c => <section key={c.challenge_id} className="rounded-2xl border border-[#2a405b] bg-[#101a2d] p-5"><div className="flex items-center gap-3"><Avatar name={c.other_display_name} url={c.other_avatar_url} /><div><div className="text-[10px] font-black uppercase tracking-[.14em] text-[#65f2b5]">🤖 ScoutBot · {c.other_display_name}</div><div className="text-xs text-[#718090]">0 tickets · separate from Weekly Challenge</div></div></div>
        {c.status === 'choosing' && <><h3 className="mt-4 text-lg font-black text-white">Which type of game do you prefer?</h3><p className="mt-1 text-xs text-[#8fa0b5]">Your choice stays private until both of you choose.</p><div className="mt-4 grid gap-2 sm:grid-cols-3">{modes.map(m => <button key={m.id} onClick={() => void prefer(c.challenge_id, m.id)} disabled={busy === c.challenge_id} className="rounded-xl border border-[#2b3e58] bg-[#0c1627] px-3 py-3 text-xs font-black text-white">{m.icon} {m.title}</button>)}</div></>}
        {c.status === 'negotiating' && <><h3 className="mt-4 text-lg font-black text-white">You chose different modes.</h3><p className="mt-1 text-xs text-[#8fa0b5]">Choose again or let ScoutBot randomly pick between your choices.</p><div className="mt-4 grid gap-2 sm:grid-cols-3">{modes.map(m => <button key={m.id} onClick={() => void prefer(c.challenge_id, m.id)} className="rounded-xl border border-[#3a4b63] px-3 py-2 text-xs font-bold">{m.icon} {m.title}</button>)}</div><button onClick={() => void randomizeMode(c.challenge_id)} className="mt-3 rounded-xl bg-[#59e8f3] px-4 py-2.5 text-xs font-black text-[#07101f]">🎲 RANDOM MODE</button></>}
