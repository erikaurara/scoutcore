...
const modeInfo = {
  weekly_h2h: { icon: 'swords', eyebrow: 'Mode 1 · 1 vs 1', title: 'Weekly Head-to-Head', description: 'Your normal weekly IXMetrics picks are compared automatically. No extra predictions.', accent: '#50eaf4' },
...
const flowSteps: Record<FriendsChallengeMode, { title: string; copy: string }[]> = {
  weekly_h2h: [
    ...
    { title: 'Make Weekly Picks', copy: 'Normal IXMetrics picks' },
...
const AutoSettlement = ({ kind, challengeId, game, onSettled }: { kind: 'same' | 'team'; challengeId: string; game: GameChoice; onSettled: () => Promise<void> }) => {
  ...
  const [status, setStatus] = useState('IXMetrics checks the verified MLB result automatically.');
...
const WeeklyMatchup = ({ challenge, onRefresh, onOpenWeeklyPicks }: { challenge: Challenge; onRefresh: () => Promise<void>; onOpenWeeklyPicks: () => void }) => {
  ...
  return <div className="mt-4 rounded-xl border border-[#2b3e58] bg-[#0a1728] p-4"><div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 text-center"><div><div className="text-[10px] font-black text-[#59e8f3]">YOU</div><div className="mt-1 text-3xl font-black text-white">{stats.my_points}</div><div className="text-[10px] text-[#8fa0b5]">{stats.my_correct}/{stats.my_total} · {mineAccuracy}%</div></div><div className="text-xs font-black text-[#65788f]">VS</div><div><div data-i18n-user-content className="truncate text-[10px] font-black text-white">{challenge.other_display_name}</div><div className="mt-1 text-3xl font-black text-white">{stats.other_points}</div><div className="text-[10px] text-[#8fa0b5]">{stats.other_correct}/{stats.other_total} · {otherAccuracy}%</div></div></div><div className="mt-3 border-t border-[#263951] pt-3 text-center text-xs font-black text-[#65f2b5]">{tied ? 'TIED RIGHT NOW' : mineLeading ? 'YOU’RE LEADING' : <><span data-i18n-user-content>{challenge.other_display_name.toUpperCase()}</span> IS LEADING</>}</div><p className="mt-1 text-center text-[10px] text-[#8292a7]">Every completed normal Weekly Challenge prediction updates this score automatically.</p><button type="button" onClick={onOpenWeeklyPicks} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl bg-[#50eaf4] px-4 py-3 text-xs font-black uppercase text-[#06101f]"><span className="material-symbols-outlined text-[18px]">edit_note</span>{Number(stats.my_total || 0) ? 'Continue normal weekly picks' : 'Open normal weekly picks'}</button>{weekEnd && Date.now() >= weekEnd && <button type="button" onClick={async () => { if (supabase) await supabase.rpc('complete_friend_weekly_challenge', { p_challenge_id: challenge.challenge_id }); await onRefresh(); }} className="mt-3 w-full rounded-lg border border-[#65f2b5]/45 px-3 py-2 text-[10px] font-black uppercase text-[#65f2b5]">Save final weekly result</button>}</div>;
};

const TeamRoster = ({ members }: { members: TeamMember[] }) => (
  <div className="grid gap-3 sm:grid-cols-2">{([1, 2] as const).map((team) => <div key={team} className="rounded-xl border border-[#2b4059] bg-[#091729] p-3"><div className={`text-[10px] font-black uppercase tracking-[.14em] ${team === 1 ? 'text-[#8bed68]' : 'text-[#f6bf4f]'}`}>{team === 1 ? 'Team Cyan' : 'Team Gold'}</div><div className="mt-2 space-y-2">{members.filter((member) => member.team === team).map((member) => <div key={member.profile_id} className="flex items-center gap-2 rounded-lg bg-[#101c2f] p-2"><Avatar name={member.display_name} url={member.avatar_url} small /><div className="min-w-0 flex-1"><div data-i18n-user-content className="truncate text-xs font-black text-white">{member.display_name}</div><div className="text-[9px] uppercase text-[#8292a7]">{member.role}{member.submitted ? ' · picks locked' : ''}</div></div><span className={`text-[9px] font-black uppercase ${member.invite_status === 'accepted' ? 'text-[#8bed68]' : member.invite_status === 'declined' ? 'text-[#ff7d8b]' : 'text-[#f6bf4f]'}`}>{member.score != null ? `${member.score}/5` : member.invite_status}</span></div>)}{!members.some((member) => member.team === team) && <div className="rounded-lg border border-dashed border-[#31455f] px-3 py-5 text-center text-[10px] text-[#718198]">Waiting for this duo</div>}</div></div>)}</div>
);
...
export const FriendsChallengeView: React.FC<{ onBack: () => void; onOpenWeeklyPicks: () => void; initialMode?: FriendsChallengeMode | null; initialView?: View }> = ({ onBack, onOpenWeeklyPicks, initialMode = null, initialView = 'play' }) => {
  ...
  const runAction: ActionRunner = async (key, action, after) => { setBusy(key); setError(''); try { const { error: actionError } = await action(); if (actionError) throw actionError; after?.(); await load(); return true; } catch (caughtError: any) { setError(caughtError?.message || 'IXMetrics could not finish that action.'); return false; } finally { setBusy(null); } };
