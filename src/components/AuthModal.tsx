    ...
    <div className="fixed inset-0 z-[70] bg-[#060e20]/80 backdrop-blur-md flex items-center justify-center p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-2xl border border-[#3b494b]/40 bg-[#171f33] shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
          ...
          {mode === 'signup' && <div><label className="text-[10px] uppercase text-[#849495]">Display name</label><input value={name} onChange={(e)=>setName(e.target.value)} autoComplete="name" className="mt-1 w-full rounded-lg bg-[#0f182b] border border-[#34425a] px-3 py-2 text-sm outline-none focus:border-[#00f0ff]" placeholder="Your name" /></div>}
          <div><label className="text-[10px] uppercase text-[#849495]">Email</label><input type="email" value={email} onChange={(e)=>setEmail(e.target.value)} autoComplete="email" className="mt-1 w-full rounded-lg bg-[#0f182b] border border-[#34425a] px-3 py-2 text-sm outline-none focus:border-[#00f0ff]" placeholder="you@example.com" /></div>
          <div>
            <label className="text-[10px] uppercase text-[#849495]">Password</label>
          ...
          <button onClick={()=>{setMode(mode === 'signin' ? 'signup' : 'signin'); setShowPassword(false); setError(null); setMessage(null);}} className="w-full text-xs text-[#9aabad] hover:text-[#00f0ff]">{mode === 'signin' ? 'New to ScoutCoreMLB? Create an account' : 'Already have an account? Sign in'}</button>
        </div>
      </div>
    </div>
  );
