...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose }) => {
  ...
  const submit = async () => {
    ...
    if (!isSupabaseConfigured || !supabase) {
      setError('Account login is designed and ready, but Supabase still needs to be connected in Netlify before real accounts can be created.');
      return;
    }
    ...
    <div className="fixed inset-0 z-[70] bg-[#060e20]/80 backdrop-blur-md flex items-center justify-center p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-2xl border border-[#3b494b]/40 bg-[#171f33] shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="p-5 border-b border-[#3b494b]/30 flex items-center justify-between">
          <div><div className="text-[10px] text-[#00f0ff] uppercase tracking-[.2em]">ScoutCore Account</div><h2 className="text-xl font-bold text-[#dae2fd] mt-1">{mode === 'signin' ? 'Welcome back' : 'Create your account'}</h2></div>
        ...
        <div className="p-5 space-y-4">
          ...
          <button onClick={submit} disabled={loading || !email.trim() || !password} className="w-full py-3 rounded-xl bg-[#00f0ff] text-[#00363a] font-bold text-xs uppercase disabled:opacity-50">{loading ? 'Please wait…' : mode === 'signin' ? 'Sign in' : 'Create free account'}</button>
