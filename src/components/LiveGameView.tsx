  ...
  return <div className="min-h-screen bg-[#08111f] text-[#dae2fd] pb-10">
    <section className="border-b border-[#26364e] bg-[#0b1424]">
      ...
      <div className="max-w-6xl mx-auto px-2 sm:px-5 flex items-center gap-1 border-t border-[#1e2d43] overflow-hidden">
        {(['live','box','matchup','insights'] as LiveTab[]).map(item => <button key={item} onClick={()=>setTab(item)} className={`flex-1 min-w-0 py-3 text-[11px] sm:text-sm font-bold uppercase border-b-2 ${tab===item?'border-[#00e6f4] text-[#e8fbff]':'border-transparent text-[#8998ad]'}`}>{item === 'box' ? 'Box Score' : item}</button>)}
      </div>
    </section>
    ...
    {!loading&&feed&&tab==='box'&&<main className="max-w-6xl mx-auto px-3 sm:px-5 py-4 space-y-4">
      ...
      <div className="rounded-2xl border border-[#2b405b] bg-[#0d1727] overflow-hidden">
        <div className="flex border-b border-[#26364e]"><button onClick={()=>setBoxSide('away')} className={`flex-1 py-3 text-sm font-bold ${boxSide==='away'?'text-white bg-[#152237]':'text-[#8fa0b7]'}`}>{awayTeam?.teamName??awayTeam?.name??'Away'}</button><button onClick={()=>setBoxSide('home')} className={`flex-1 py-3 text-sm font-bold ${boxSide==='home'?'text-white bg-[#152237]':'text-[#8fa0b7]'}`}>{homeTeam?.teamName??homeTeam?.name??'Home'}</button></div>
        <div className="p-3 sm:p-4"><p className="text-xs font-bold text-[#00e6f4] mb-2">BATTERS — {sideTeam?.abbreviation??sideTeam?.name??''}</p><BatterTable players={batters} /></div>
      </div>
...
const BatterTable=({players}:{players:any[]})=><div className="w-full text-[10px] sm:text-xs"><div className="grid grid-cols-[minmax(90px,1.6fr)_repeat(5,minmax(28px,.45fr))] sm:grid-cols-[minmax(130px,1.7fr)_repeat(7,minmax(40px,.5fr))] gap-1 px-2 py-2 text-[#8fa0b7] border-b border-[#26364e]"><span>PLAYER</span><span className="text-center">AB</span><span className="text-center">R</span><span className="text-center">H</span><span className="text-center">RBI</span><span className="text-center">K</span><span className="hidden sm:block text-center">BB</span><span className="hidden sm:block text-center">OPS</span></div>{players.length?players.map((p:any)=>{const s=p?.stats?.batting??{};return <div key={p?.person?.id} className="grid grid-cols-[minmax(90px,1.6fr)_repeat(5,minmax(28px,.45fr))] sm:grid-cols-[minmax(130px,1.7fr)_repeat(7,minmax(40px,.5fr))] gap-1 px-2 py-2.5 border-b border-[#1c2b40] last:border-0 items-center"><span className="font-semibold truncate">{p?.person?.fullName??'—'} <span className="text-[#7f8ea3] font-normal">{p?.position?.abbreviation??''}</span></span><span className="text-center">{num(s.atBats,0)}</span><span className="text-center">{num(s.runs,0)}</span><span className="text-center">{num(s.hits,0)}</span><span className="text-center">{num(s.rbi,0)}</span><span className="text-center">{num(s.strikeOuts,0)}</span><span className="hidden sm:block text-center">{num(s.baseOnBalls,0)}</span><span className="hidden sm:block text-center">{s.ops??'—'}</span></div>}):<p className="py-4 text-[#8fa0b7]">Batting lines will populate when available.</p>}</div>;
