...
export const LiveGameView: React.FC<LiveGameViewProps> = ({ selectedGame, onOpenMatchup, onBack }) => {
  ...
  return <div className="min-h-screen bg-[#08111f] text-[#dae2fd] pb-10">
    ...
    {!loading&&feed&&tab==='insights'&&<main className="max-w-6xl mx-auto px-3 sm:px-5 py-4 space-y-4">
      ...
      <section className="rounded-2xl border border-[#00e6f4]/25 bg-[#0d1727] p-4"><p className="text-[10px] font-bold text-[#00e6f4]">IXMETRICS LIVE NOTE</p><p className="text-sm text-[#b9c5d8] mt-2 leading-6">This screen uses the game feed for live score, pitch locations, count, baserunners, current players and box-score data. IXMetrics does not turn the live feed into a guaranteed outcome prediction.</p></section>
    </main>}
  </div>;
};
