...
const SCOUT_LEVELS: ScoutLevel[] = [
  ...
  { name: 'IX All-Star', min: 5000, next: null, icon: 'stars', accent: '#ff9d69' },
];
...
export const ProfileView: React.FC<ProfileViewProps> = ({ onOpenChallenge, onOpenSettings }) => {
  ...
  const nextLevelCopy = scoutLevel.next == null ? 'Highest Analyst level reached' : `${Math.max(0, scoutLevel.next - challengeSummary.points)} points to next level`;
  ...
  return (
    <div className="min-h-screen bg-[#0b1326] px-4 py-6 text-[#dae2fd] sm:px-6 lg:px-8">
      <div className="mx-auto max-w-5xl space-y-5">
          ...
          <div className="px-5 pb-5 sm:px-7 sm:pb-7">
            <div className="-mt-12 flex flex-col gap-4 sm:-mt-14 sm:flex-row sm:items-end sm:justify-between">
              <div className="flex items-end gap-4">
                ...
                <div className="pb-1"><div className="text-xs text-[#91a0b5]">Joined {joined}</div><h1 className="mt-1 text-3xl font-extrabold text-white">{displayName.trim() || user.email?.split('@')[0] || 'IXMetrics User'}</h1><div className="mt-1 text-sm text-[#849495]">{user.email}</div></div>
              </div>
              <button onClick={onOpenChallenge} className="min-w-[230px] rounded-2xl border border-[#00f0ff]/30 bg-[#071827]/85 p-4 text-left shadow-[0_12px_40px_rgba(0,240,255,.08)] transition hover:border-[#00f0ff]/55">
                <div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/10 bg-white/5" style={{ color: scoutLevel.accent }}><span className="material-symbols-outlined">{scoutLevel.icon}</span></div><div className="min-w-0 flex-1"><div className="text-[9px] font-extrabold uppercase tracking-[.16em] text-[#7df4ff]">Analyst Level</div><div className="mt-0.5 truncate text-base font-extrabold text-white">{scoutLevel.name}</div><div className="text-[10px] text-[#91a0b5]">{challengeSummary.points.toLocaleString()} IX Points</div></div></div>
        ...
        <section className="rounded-2xl border border-[#2a405b] bg-[#0c1627] p-4 text-[11px] leading-5 text-[#718090]">IX Points measure Challenge performance only. They have no cash value, cannot be bought, exchanged, transferred or withdrawn. IXMetrics analysis scores and IX Points are separate systems.</section>
      </div>
    </div>
  );
};
