...
function buildPregameSignals(schedule: any, selectedDate: string) {
  ...
  return games.map((game: any) => {
    ...
    return { id:`pre-${selectedDate}-${game.gamePk}`, gamePk:game.gamePk, player:`${away?.name ?? 'Away Team'} at ${home?.name ?? 'Home Team'}`, team:gameTime(game.gameDate), type:'GAME PREVIEW', severity:bothStarters?'READY':'WAITING', image:teamLogo(away?.id), secondaryImage:teamLogo(home?.id), short:`${awayPitcher?.fullName ?? 'TBD'} vs ${homePitcher?.fullName ?? 'TBD'}`, description:`Probable starters: ${awayPitcher?.fullName ?? 'TBD'} vs ${homePitcher?.fullName ?? 'TBD'}. IXMetrics will use confirmed lineups, pitcher handedness and current player data as they become available.`, confidence:bothStarters?'STARTERS POSTED':'STARTERS TBD' };
  });
}
...
export const ScoutingFeedView:React.FC=()=>{
  ...
  return <div className="min-h-screen bg-[#0b1326] text-[#dae2fd] p-8 space-y-6">
    <div className="flex flex-wrap justify-between gap-4 border-b border-[#3b494b]/20 pb-6"><div><span className="font-label-caps text-xs text-[#00f0ff]">IXMETRICS INTELLIGENCE</span><h1 className="font-display-lg text-4xl">Insights Feed</h1><p className="text-sm text-[#849495] mt-1">Quick visual reports. Click any card for the full scouting detail.</p></div><div className="flex flex-col items-end gap-2"><div className="flex gap-2 bg-[#131b2e] p-1.5 rounded-xl">{['PRE-GAME','POST-GAME'].map(item=><button key={item} onClick={()=>setMode(item as 'PRE-GAME'|'POST-GAME')} className={`px-4 py-1.5 rounded-lg text-xs font-bold ${mode===item?'bg-[#00f0ff] text-[#00363a]':'text-[#849495]'}`}>{item}</button>)}</div><span className="text-xs text-[#849495]">{labelDate(selectedDate)} · ET</span></div></div>
